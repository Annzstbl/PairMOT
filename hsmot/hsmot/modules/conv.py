...
def autopad(
    kernel_size: Union[int, Tuple[int, ...]], padding: Optional[Union[int, Tuple[int, ...]]] = None, dilation: int = 1
) -> Union[int, Tuple[int, ...]]:
    """
    自动计算填充大小以保持输出形状不变。
...
class ConvMSI(nn.Module):
    ...
    def __init__(
        ...
        # 第一个3D卷积层
        self.conv3d = nn.Conv3d(in_channels=c1, out_channels=c2, kernel_size=k, stride=s, padding=p, bias=False)
        ...
        # 深度方向融合卷积（深度可分离卷积）
        self.fuse = nn.Conv3d(
            in_channels=c2, out_channels=c2, kernel_size=(temporal_output_size, 1, 1), groups=groups or c2, bias=False
        )
    ...
    def _calculate_temporal_output_size(
        self, c3: int, temporal_padding: int, temporal_kernel: int, temporal_stride: int
    ) -> int:
        """
        计算3D卷积后时间维度的输出大小。
        ...
        Returns:
            时间维度的输出大小
        """
        return math.floor((c3 + 2 * temporal_padding - (temporal_kernel - 1) - 1) / temporal_stride) + 1
...
class ConvMSI_SE(nn.Module):
    ...
    def __init__(
        ...
        final_act: bool = True,
        reduction: int = 4,
        return_before_sigmoid: bool = False,
    ) -> None:
        super().__init__()
        assert c1 == 1, f"c1 must be 1, got {c1}"
        assert c3 > 1, f"c3 must be > 1, got {c3}"
