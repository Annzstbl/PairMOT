...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        for transform in transforms:
            assert not isinstance(transform, dict), f'Yet, Do not support transform type {type(transform)}'
...
class LoadAnnotations:
    ...
    def __init__(self,
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        assert self.poly2mask == False, "Do not support poly2mask yet."
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            results[key] = mmcv.imnormalize_(results[key].astype(np.float32), self.mean, self.std,
                                            self.to_rgb)
