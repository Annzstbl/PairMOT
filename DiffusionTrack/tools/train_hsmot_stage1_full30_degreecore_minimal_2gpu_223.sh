#!/usr/bin/env bash
# Manual two-GPU full-HSMOT Stage-1 run.  No queueing or automatic restart.
set -euo pipefail
...
EXP=yolo11l_diffusion_det_hsmot_full30_degreecore_minimal
LOG=${RUNTIME}/logs/${EXP}_20260724.log
...
if [[ -e "${RUNTIME}/work_dirs/${EXP}" ]]; then
    echo "refusing to overwrite existing workdir: ${RUNTIME}/work_dirs/${EXP}" >&2
    exit 1
fi

cd "${REPO}"
exec "${ENV}/bin/python" tools/train.py \
    -f exps/example/mot/yolo11l_diffusion_det_hsmot_full30_degreecore_minimal.py \
