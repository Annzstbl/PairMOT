...
if __name__ == '__main__':
    ...
    # 默认类型为 None 的项会用 nargs='+'，会把「单路径」收成 list（如 ['/a/b']），破坏 OUTPUT_FOLDER 等应为 str 的字段。
    # TRACKERS_TO_EVAL / METRICS 等需要 list，保持 nargs='+'；OUTPUT_FOLDER 必须单独注册为单字符串参数。
    ...
    for setting in args.keys():
        if args[setting] is not None:
            if type(config[setting]) == type(True):
                ...
                x = args[setting]
            config[setting] = x
    # 旧版曾把 OUTPUT_FOLDER 当 None→nargs+，若读到单项 list 则收敛为 str
