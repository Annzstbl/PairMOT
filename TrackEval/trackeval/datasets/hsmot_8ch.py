...
class HSMOT_8ch(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'TRACKERS_FOLDER': '/data3/litianhao/hsmot',  # Trackers location (CLI 一般会覆盖)
            # 空字符串：与 None 同义，结果写到 TRACKERS_FOLDER。勿填死路径，否则未传 --OUTPUT_FOLDER 时 eval 会落到别处。
            'OUTPUT_FOLDER': '',  # Where to save eval results (empty or None => same as TRACKERS_FOLDER)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if is_gt:
            ...
            self.seq_lengths[seq] = len(os.listdir(img_path)) // 3 if "npy2jpg" in self.img_fol else len(os.listdir(img_path))
