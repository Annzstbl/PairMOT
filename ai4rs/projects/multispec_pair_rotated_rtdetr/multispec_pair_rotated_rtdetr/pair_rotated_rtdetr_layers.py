...
class PairRotatedRTDETRTransformerDecoder(DinoTransformerDecoder):
    ...
    def __init__(self,
        ...
        self.pair_shared_terminal_transport_shared_metric_product_tangent_refinement_decoder = (
            bool(
                pair_shared_terminal_transport_shared_metric_product_tangent_refinement_decoder))
        self.pair_shared_terminal_transport_householder_product_tangent_refinement_decoder = (
            bool(
                pair_shared_terminal_transport_householder_product_tangent_refinement_decoder))
        ...
        self.pair_shared_terminal_transport_axis_frenet_product_tangent_refinement_decoder = (
            bool(
                pair_shared_terminal_transport_axis_frenet_product_tangent_refinement_decoder))
        self.pair_shared_terminal_transport_covariant_frenet_product_tangent_refinement_decoder = (
            bool(
                pair_shared_terminal_transport_covariant_frenet_product_tangent_refinement_decoder))
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.pair_shared_shape_refinement_decoder:
                ...
                tmp_prev, tmp_curr = (
                  ...
                  and lid == self.num_layers - 1):
                num_dn = max(tmp_prev.shape[1] - self.num_queries, 0)
                # Parallel-transport the complete pair detail with a minimal
                # orthogonal Householder map instead of discarding its
                # transverse component through a rank-one projection.  The
                # center and shape bundles remain factorized, and the map is
                # norm preserving, parameter free, and swap equivariant.
                tmp_prev, tmp_curr = (
                    self._pair_householder_center_tangent_residual(
                        tmp_prev, tmp_curr, reference_prev, reference_curr,
                        num_dn))
                tmp_prev, tmp_curr = (
                    self._pair_householder_shape_tangent_residual(
                        tmp_prev, tmp_curr, reference_prev, reference_curr,
                        num_dn))
            elif ((
                    self.
                ...
                tmp_prev, tmp_curr = (
                  ...
                  and lid == self.num_layers - 1):
                num_dn = max(tmp_prev.shape[1] - self.num_queries, 0)
                # Parallel-transport both complete center updates into the
                # midpoint Frenet frame before separating common and detail
                # motion.  Only transverse *inconsistency* is suppressed;
                # the full common two-vector is transported back to the two
                # endpoint frames.  Shape transport is unchanged.
                tmp_prev, tmp_curr = (
                    self._pair_transport_covariant_frenet_center_tangent_residual(
                        tmp_prev, tmp_curr, reference_prev, reference_curr,
                        num_dn))
                tmp_prev, tmp_curr = (
                    self._pair_transport_shape_tangent_residual(
    ...
    @staticmethod
    def _householder_transport_detail(
            detail: Tensor, transport: Tensor) -> Tensor:
        """Align detail to a transport axis with an orthogonal reflection.

        The sign of the target axis is chosen to require the smaller turn.
        The reflector is built from detached directions, so its backward map
        is itself orthogonal.  Degenerate details or transport axes use the
        identity map.  Unlike a rank-one projection, no detail energy is
        deleted or amplified.
        """
        if detail.shape != transport.shape:
            raise ValueError(
                'Householder detail and transport must have equal shapes')
        ...
        reflector_direction = detail_direction - target_direction
        reflector_energy = reflector_direction.square().sum(
            dim=-1, keepdim=True)
        reflected = detail - 2.0 * reflector_direction * (
            (detail * reflector_direction).sum(dim=-1, keepdim=True)
            / reflector_energy.clamp_min(1e-12))
    ...
    def _pair_householder_center_tangent_residual(
            ...
            reference_prev: Tensor, reference_curr: Tensor,
            num_dn: int) -> Tuple[Tensor, Tensor]:
        """Transport complete center detail onto established translation.

        Center increments use the established per-frame width/height metric.
        Their common component is retained, while the complete antisymmetric
        detail is minimally reflected onto the detached reference-motion
        axis.  This preserves detail norm instead of projecting it away.
        """
        if residual_prev.shape != residual_curr.shape:
            raise ValueError(
                'pair Householder center refinement requires aligned '
                'residuals')
        if reference_prev.shape != reference_curr.shape:
            raise ValueError(
                'pair Householder center refinement requires aligned '
        ...
        if residual_prev.shape != reference_prev.shape:
            raise ValueError(
                'pair Householder center refinement requires residual and '
                'reference shapes to match')
        if residual_prev.shape[-1] != 5:
            raise ValueError(
                'pair Householder center refinement requires 5D boxes')
        if num_dn < 0 or num_dn > residual_prev.shape[1]:
            raise ValueError(
                f'invalid DN prefix length {num_dn} for pair residuals')
    ...
    @classmethod
    def _pair_householder_shape_tangent_residual(
            cls, residual_prev: Tensor, residual_curr: Tensor,
            reference_prev: Tensor, reference_curr: Tensor,
            num_dn: int) -> Tuple[Tensor, Tensor]:
        """Transport complete log-size/angle detail without energy loss."""
        if residual_prev.shape != residual_curr.shape:
            raise ValueError(
                'pair Householder shape refinement requires aligned '
                'residuals')
        if reference_prev.shape != reference_curr.shape:
            raise ValueError(
                'pair Householder shape refinement requires aligned '
        ...
        if residual_prev.shape != reference_prev.shape:
            raise ValueError(
                'pair Householder shape refinement requires residual and '
                'reference shapes to match')
        if residual_prev.shape[-1] != 5:
            raise ValueError(
                'pair Householder shape refinement requires 5D boxes')
        if num_dn < 0 or num_dn > residual_prev.shape[1]:
            raise ValueError(
                f'invalid DN prefix length {num_dn} for pair residuals')
    ...
    @staticmethod
    def _pair_transport_covariant_frenet_center_tangent_residual(
            residual_prev: Tensor, residual_curr: Tensor,
            reference_prev: Tensor, reference_curr: Tensor,
            num_dn: int) -> Tuple[Tensor, Tensor]:
        """Transport center updates through a discrete Frenet connection.

        The established product tangent compares width/height-normalized
        updates directly in image axes and projects their antisymmetric part
        onto one reference chord.  For curved motion, however, the two
        endpoint tangent spaces differ by the observed orientation turn.
        This operation first rotates both complete updates into the midpoint
        Frenet frame.  It retains their full common two-vector and only
        projects the aligned antisymmetric inconsistency onto the chord,
        before parallel-transporting both results back to their endpoints.

        Zero turn is exactly the original product-tangent operation.  Frame
        reversal swaps the endpoint transports, so the map is swap
        equivariant.  It is parameter free, class agnostic, preserves the DN
        prefix, and adds only terminal elementwise trigonometry.
        """
        if residual_prev.shape != residual_curr.shape:
            raise ValueError(
                'pair covariant-Frenet center refinement requires aligned '
                'residuals')
        if reference_prev.shape != reference_curr.shape:
            raise ValueError(
                'pair covariant-Frenet center refinement requires aligned '
        ...
        if residual_prev.shape != reference_prev.shape:
            raise ValueError(
                'pair covariant-Frenet center refinement requires residual '
                'and reference shapes to match')
        if residual_prev.shape[-1] != 5:
            raise ValueError(
                'pair covariant-Frenet center refinement requires 5D boxes')
        if num_dn < 0 or num_dn > residual_prev.shape[1]:
            raise ValueError(
                f'invalid DN prefix length {num_dn} for pair residuals')
        ...
        aligned_prev = rotate(tangent_prev, half_turn)
        aligned_curr = rotate(tangent_curr, -half_turn)
        common_aligned = 0.5 * (aligned_prev + aligned_curr)
        detail_aligned = 0.5 * (aligned_curr - aligned_prev)
        chord_energy = chord.square().sum(
            dim=-1, keepdim=True).clamp_min(1e-6)
        transported_detail = chord * (
            (detail_aligned * chord).sum(dim=-1, keepdim=True)
            / chord_energy)
        tangent_prev = rotate(
            common_aligned - transported_detail, -half_turn)
        tangent_curr = rotate(
            common_aligned + transported_detail, half_turn)
