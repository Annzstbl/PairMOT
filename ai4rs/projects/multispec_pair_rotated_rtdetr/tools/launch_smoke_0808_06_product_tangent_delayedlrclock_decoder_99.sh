...
REPO=${PAIRMOT_REPO:-/data/users/wangying01/lth/PairMOT_0808_06_delayedlr_99/ai4rs/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0808_06_product_tangent_delayedlrclock_decoder_4iter_smoke_99.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_99/smoke_0808_06_product_tangent_delayedlrclock_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0808_06 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] 0808_06 DDP smoke GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
