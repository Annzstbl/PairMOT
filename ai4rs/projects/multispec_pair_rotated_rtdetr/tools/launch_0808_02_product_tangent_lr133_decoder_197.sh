...
REPO=${PAIRMOT_REPO:-/data/users/litianhao/PairMOT_0808_02_lr133_197/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0808_02_iterative_cls_terminal_transport_product_tangent_lr133_decoder_197.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_197/0808_02_final_product_tangent_lr133_72e_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0808_02 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0808_02 GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
exec "${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29884 \
