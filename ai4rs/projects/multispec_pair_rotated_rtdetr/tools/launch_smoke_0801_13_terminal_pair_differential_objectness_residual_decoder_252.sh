...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0801_13_terminal_pair_differential_objectness_residual_decoder_4iter_smoke_252.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_252/smoke_0801_13_terminal_pair_differential_objectness_residual_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0801_13 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh 0801_13 real-data DDP smoke" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29832 \
...
"${PYTHON_ROOT}/python" \
    projects/multispec_pair_rotated_rtdetr/tools/check_terminal_pair_differential_objectness_residual_checkpoint.py \
