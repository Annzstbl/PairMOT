#!/usr/bin/env python3
"""Create synthetic HSMOT single-frame data for overfit acceptance.

Produces one sequence with ``num_frames`` frames (default 10).
Tracks:
  - id=1: persistent (visible in every frame)
...
def create_hsmot_single_overfit_data(
    root: str,
    num_frames: int = 10,
    img_size: int = 128,
) -> str:
    """Create train split under ``root`` (val/test reuse the same split)."""
    seq_name = 'single-overfit-1'
    ...
    with open(osp.join(imagesets_dir, 'train.txt'), 'w',
              encoding='utf-8') as f:
        f.write(f'{seq_name}\n')
    print(f'Created single-frame overfit data: {root} '
          f'({num_frames} frames, 3 tracks)')
    return root


def parse_args():
    parser = argparse.ArgumentParser(
        description='Create synthetic HSMOT single-frame overfit dataset')
    parser.add_argument(
        '--root',
