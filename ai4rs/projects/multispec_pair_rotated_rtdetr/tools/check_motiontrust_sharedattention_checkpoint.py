...
assert len(motion) == 3, sorted(motion)
motion_maxima = {
    key: value.abs().max().item() for key, value in motion.items()
}
assert all(value > 0.0 for value in motion_maxima.values()), motion_maxima
assert all(torch.isfinite(value).all() for value in motion.values())
...
print(
    'MOTIONTRUST_SHAREDATTENTION_CHECKPOINT_OK',
    {
        'motion_maxima': motion_maxima,
