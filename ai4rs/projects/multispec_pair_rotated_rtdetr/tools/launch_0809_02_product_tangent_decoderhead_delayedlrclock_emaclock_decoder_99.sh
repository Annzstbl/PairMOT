...
REPO=${PAIRMOT_REPO:-/data/users/wangying01/lth/PairMOT_0809_02_decoderhead_delayedlr_emaclock_99/ai4rs/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0809_02_iterative_cls_terminal_transport_product_tangent_decoderhead_delayedlrclock_emaclock_decoder_99.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_99/0809_02_final_product_tangent_decoderhead_delayedlrclock_emaclock_72e_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0809_02 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0809_02 GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
exec "${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29896 \
