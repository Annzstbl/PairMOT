...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0803_02_iterative_cls_pair_shared_shape_refinement_decoder_252.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_252/0803_02_paper_base_liquid_encoder_p5temporal_dualevidence_decoder_iterativeclsdnisolatede2e_pairsharedshaperefinement_pairdn_paircoherent_le180_r18_coco_full_1200x900_bf16_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0803_02 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0803_02 commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
exec "${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29845 \
