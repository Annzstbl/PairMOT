...
REPO=${PAIRMOT_REPO:-/data1/users/litianhao01/PairMOT_framedetail_0803_08/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0803_08_iterative_cls_frame_detail_periodic_angle_decoder_4iter_smoke_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/smoke_0803_08_iterative_cls_frame_detail_periodic_angle_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0803_08 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh 0803_08 real-data smoke commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
