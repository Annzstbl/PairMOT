...
REPO=${PAIRMOT_REPO:-/data/users/litianhao/PairMOT_terminallogsize_0804_03_197/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0804_03_iterative_cls_terminal_log_size_decoder_197.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_197/0804_03_paper_base_liquid_encoder_p5temporal_dualevidence_decoder_iterativeclsdnisolatede2e_pairsharedterminallogsize_refinement_pairdn_paircoherent_le180_r18_coco_full_1200x900_bf16_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0804_03 on 197 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0804_03 GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
