...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0803_03 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0803_03 commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
