...
REPO=${PAIRMOT_REPO:-/data/users/litianhao/PairMOT_logarea_0803_10_197/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0803_10_iterative_cls_log_area_periodic_angle_decoder_4iter_smoke_197.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_197/smoke_0803_10_iterative_cls_log_area_periodic_angle_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0803_10 on 197 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh 0803_10 portability DDP smoke commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29991 \
