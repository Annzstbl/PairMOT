...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0808_07_product_tangent_staged_delayedlrclock_decoder_4iter_smoke_197.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_197/smoke_0808_07_product_tangent_staged_delayedlrclock_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0808_07 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] 0808_07 DDP smoke GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29893 \
