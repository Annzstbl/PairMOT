...
REPO=${PAIRMOT_REPO:-/data1/users/litianhao01/PairMOT_0810_04_onecycle250_178/ai4rs/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0810_04_product_tangent_onecycle250_decoder_4iter_smoke_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/smoke_0810_04_product_tangent_onecycle250_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0810_04 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] 0810_04 smoke GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
