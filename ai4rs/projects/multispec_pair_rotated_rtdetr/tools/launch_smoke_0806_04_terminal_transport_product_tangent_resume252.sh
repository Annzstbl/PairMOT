...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0806_04_iterative_cls_terminal_transport_product_tangent_decoder_4iter_smoke_252.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_252/smoke_0806_04_terminal_transport_product_tangent_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0806_04 on 252 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
test ! -e "${WORK_DIR}"
check_fixed_gpus_idle
sleep 5
check_fixed_gpus_idle
...
echo "[$(date '+%F %T')] 0806_04 DDP smoke fixed GPUs=0,1 commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29903 \
