...
REPO=${PAIRMOT_REPO:-/data1/users/litianhao01/PairMOT_quotientanisotropy_0804_16_178/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_1xb8_72e_hsmot_0804_16_iterative_cls_terminal_quotient_anisotropy_shape_consensus_decoder_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/0804_16_paper_base_liquid_encoder_p5temporal_dualevidence_decoder_iterativeclsdnisolatede2e_pairsharedterminalquotient_anisotropyshapeconsensus_pairdn_paircoherent_le180_r18_coco_full_1200x900_bf16_1xb8_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0804_16 on 178 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0804_16 GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
