...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_1xb8_72e_hsmot_0803_19_iterative_cls_terminal_full_tangent_decoder_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/0803_19_paper_base_liquid_encoder_p5temporal_dualevidence_decoder_iterativeclsdnisolatede2e_pairsharedterminalfulltangentrefinement_pairdn_paircoherent_le180_r18_coco_full_1200x900_bf16_1xb8_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0803_19 on 178 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
test ! -e "${WORK_DIR}"
mkdir -p "${WORK_DIR}"
cd "${REPO}"
...
echo "[$(date '+%F %T')] fresh formal 0803_19 GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
