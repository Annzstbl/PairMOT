...
REPO=${PAIRMOT_REPO:-/data1/users/litianhao01/PairMOT_0811_02_warmup4_cosine2667_178/ai4rs/ai4rs}
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0811_02_product_tangent_warmup4_cosine2667_decoder_4iter_smoke_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/smoke_0811_02_product_tangent_warmup4_cosine2667_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0811_02 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] 0811_02 smoke GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/python" tools/train.py "${CONFIG}" --work-dir "${WORK_DIR}" >> "${WORK_DIR}/launch.log" 2>&1
