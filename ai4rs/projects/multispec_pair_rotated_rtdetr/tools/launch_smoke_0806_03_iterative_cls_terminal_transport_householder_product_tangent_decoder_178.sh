...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0806_03_iterative_cls_terminal_transport_householder_product_tangent_decoder_4iter_smoke_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/smoke_0806_03_iterative_cls_terminal_transport_householder_product_tangent_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0806_03 on 178 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
case "${PAIRMOT_CUDA_VISIBLE_DEVICES}" in
    *','*) echo "0806_03 smoke requires exactly one GPU" >&2; exit 2 ;;
esac
...
echo "[$(date '+%F %T')] fresh 0806_03 smoke GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
"${PYTHON_ROOT}/python" tools/train.py "${CONFIG}" \
    --work-dir "${WORK_DIR}" >> "${WORK_DIR}/launch.log" 2>&1
if grep -Eiq 'Traceback|CUDA out of memory|loss: (nan|inf)|grad_norm: (nan|inf)|NCCL error|unused parameter.*error' "${WORK_DIR}/launch.log"; then exit 4; fi
