...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_1xb8_72e_hsmot_0811_02_iterative_cls_terminal_transport_product_tangent_warmup4_cosine2667_decoder_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/0811_02_final_product_tangent_warmup4_cosine2667_72e_1xb8_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0811_02 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0811_02 GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
