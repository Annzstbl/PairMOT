...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0808_08_iterative_cls_terminal_transport_product_tangent_decoderhead_adamclock_decoder_252.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_252/0808_08_final_product_tangent_decoderhead_adamclock_72e_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0808_08 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
test ! -e "${WORK_DIR}"
check_fixed_gpus_idle
sleep 5
check_fixed_gpus_idle
...
echo "[$(date '+%F %T')] fresh formal 0808_08 fixed GPUs=0,1 commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
exec "${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29892 \
