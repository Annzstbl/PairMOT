...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0810_05_product_tangent_onecycle200_decoder_4iter_smoke_252.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_252/smoke_0810_05_product_tangent_onecycle200_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0810_05 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
test ! -e "${WORK_DIR}"
check_fixed_gpus_idle
sleep 5
check_fixed_gpus_idle
...
echo "[$(date '+%F %T')] 0810_05 DDP smoke fixed GPUs=0,1 commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
