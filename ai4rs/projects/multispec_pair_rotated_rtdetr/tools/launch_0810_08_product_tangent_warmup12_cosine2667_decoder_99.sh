...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/o2_pair_rtdetr_r18vd_2xb4_72e_hsmot_0810_08_iterative_cls_terminal_transport_product_tangent_warmup12_cosine2667_decoder_99.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_99/0810_08_final_product_tangent_warmup12_cosine2667_72e_2xb4_fresh
...
trap 'status=$?; echo "[$(date "+%F %T")] formal 0810_08 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh formal 0810_08 GPUs=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
exec "${PYTHON_ROOT}/torchrun" --nproc_per_node=2 --master_port=29908 \
