...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0806_05_iterative_cls_terminal_transport_scale_orientation_product_tangent_decoder_4iter_smoke_178.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_178/smoke_0806_05_iterative_cls_terminal_transport_scale_orientation_product_tangent_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0806_05 on 178 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh 0806_05 smoke GPU=${CUDA_VISIBLE_DEVICES} commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
