...
CONFIG=projects/multispec_pair_rotated_rtdetr/configs/smoke/o2_pair_rtdetr_r18vd_0801_09_iterative_cls_dn_isolated_e2e_decoder_4iter_smoke_197.py
WORK_DIR=/data4/litianhao/PairMmot/workdir_197/smoke_0801_09_iterative_cls_dn_isolated_e2e_resume197_4iter
...
trap 'status=$?; echo "[$(date "+%F %T")] smoke 0801_09 resume197 failed: status=${status} command=${BASH_COMMAND}" >> "${WORK_DIR}/launch.log"; exit "${status}"' ERR
...
echo "[$(date '+%F %T')] fresh 0801_09 portability DDP smoke commit $(git rev-parse --short HEAD)" >> "${WORK_DIR}/launch.log"
