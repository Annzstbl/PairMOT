...
| `0717_01` | Parallel Liquid Set-Transport structural candidate | AutoDL / GPU 0,1 | `autodl_0717_01_paper_liquid_settransport_full_1200x900_bf16.py` | 99上的首次运行在epoch 2 iter 250主动取消且不计入结果；AutoDL已按同协议fresh重跑，截至2026-07-17 12:47 CST为epoch 27 iter 1000，ETA约11小时，训练与自动分析/发布/关机守护进程正常 |
...
当前 Parallel Liquid Set-Transport fresh 运行：
...
错误。早期99运行仍不计入最终结果。截至2026-07-17 12:47 CST，AutoDL运行已到
epoch 27 iter 1000，`time=0.8799 s/iter`、`memory=10693 MB/rank`，训练ETA约11小时；
Set-Transport强度已达到`1.000`，hard预览仍为`unique_sets=8.00`、
`max_set_repeat=1.00`。训练进程和自动等待18/18异步TrackEval、生成报告、推送GitHub并
关机的守护进程均正常存活。
