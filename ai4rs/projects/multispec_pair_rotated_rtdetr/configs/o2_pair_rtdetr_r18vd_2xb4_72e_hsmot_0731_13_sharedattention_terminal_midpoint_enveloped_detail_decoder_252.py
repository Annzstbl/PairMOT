"""0731_13: terminal-only frame detail with midpoint-preserving regression.

The recurrent decoder state, auxiliary outputs, and intermediate references
remain exactly on the shared-attention parent path.  Only the terminal
classification features receive the bounded swap-odd frame correction.  The
terminal box correction is converted to an antisymmetric 5D logit residual so
the added detail cannot move the pair midpoint through independent nonlinear
regression heads.
"""
...
work_dir = (
    ...
    '0731_13_paper_base_liquid_encoder_p5temporal_dualevidence_'
    'decoder_sharedattention_terminalmidpointenvelopeddetail_'
