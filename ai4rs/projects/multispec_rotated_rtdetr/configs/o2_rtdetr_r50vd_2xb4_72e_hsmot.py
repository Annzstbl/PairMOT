from ai4rs.projects.rotated_rtdetr.rotated_rtdetr.rtdetr_layers import RTDETRFPN
...
model = dict(
    ...
    data_preprocessor=dict(
        type=MultispecDetDataPreprocessor,
        mean=hsmot_mean,
        std=hsmot_std,
        bgr_to_rgb=False,
        pad_size_divisor=32,
        ...
        batch_augments=None),
    backbone=dict(
        type=MultispecResNetV1dPaddle3DSE,
        in_channels=8,
        num_spectral=8,
        se_reduction=4,
        depth=50,
    ...
    encoder=dict(
        ...
        fpn_cfg=dict[str, list[int] | int | float | dict[str, str | bool] | type[RTDETRFPN]](
            type=RTDETRFPN,
            in_channels=[256, 256, 256],
            out_channels=256,
            expansion=1.0,
