...
| 服务器 | 实验 | 状态 | 结构与判定方式 |
| --- | --- | --- | --- |
| 99 GPU 0,1 | `0730_05 ... decoder_commonmotion ... fresh` | `RUNNING`；epoch 4 iter 800，约 1.05 s/iter，loss 10.7503、grad_norm 55.8358 均有限，显存约 19.2 GB/rank | 以 `0727_01` 为严格父配置；共享 query/分类路径不变，用两帧 cross-attention 差分和周期角度 reference 位移预测零初始化的 5D 反对称运动修正。4-iter 真数据 DDP smoke 已通过，三层新权重均从 0 更新到约 `3.98e-4`。正在接近首个 epoch 4 正式判断点。 |
| 178 GPU 0 | `0730_06 ... decoder_sharedevidence ... fresh` | `RUNNING`；epoch 4 iter 800，约 0.92 s/iter，loss 10.4795、grad_norm 41.9600 均有限，MMEngine 峰值约 21.7 GB | 以 `0727_01` 为严格父配置；将两帧 cross-attention 的相对不一致度作为交换不变证据，零初始化地注入共享 query，直接服务 cls 与两帧框。4-iter 真数据 smoke 已通过，三层新权重均从 0 更新到约 `4.00e-4`。正在接近首个 epoch 4 正式判断点。 |
...
- 178、252、99 以及 197 的干净运维副本当前均位于提交 `1306d3b`；其中 `eb9c440` 为公共运动 decoder，`25fe052` 为共享证据 decoder，`a8d8ded` 增加两者组合验证。
...
- 197 后续运维使用既有且干净的 `/data/users/litianhao/PairMOT_sync_3cb888d`；该副本已 fast-forward 至 `1306d3b` 并在 197 环境通过 36/36 decoder 测试。历史目录 `/data/users/litianhao/PairMOT` 保持在原状态（39 个已修改、238 个未跟踪项），未覆盖、未清理，也不作为新实验代码源。

## 下一步

1. 在 epoch 4 获取 `0730_05` 的 cls/det HOTA、DetA、AssA 和检测 AP，并与 `0727_01` 同点比较。
2. 对 `0730_06` 同样在 epoch 4 做首轮 TrackEval；对真正的结构候选，避免仅依据历史上不可靠的早期 det 激增判断，必要时延长到 epoch 8 复核。
3. 仅当独立结构分别显示 cls 或 det 的可靠改善时，才在空闲的 197 上构建组合模型；否则保留论文现有三段递进主线。
