...
## 2026-08-14 01:42 CST：残留进程复核

- 197与252现场均无PairMOT训练或评测进程，全部GPU空闲。197仅残留一个旧`0812_01`审计脚本，长期轮询不存在的`val_track_0005/async_done.json`且不占GPU；已精确终止PID `63474`，未触碰实验产物或无关进程。
...
- 197/e72最终为`55.379/62.180=117.559`，cls通过目标`0.116`，det/sum分别差`0.419/0.303`；DetA/AssA为`45.576/68.983`与`54.068/73.850`，pair mAP/AP50为`0.3190/0.5343`。2915个checkpoint张量全有限，51检测文件、28 CSV/108 TrackEval文件完整，正式进程退出且GPU4/5释放，严格失败。
- 252/e64为`55.211/62.599=117.810`，det恰好达到目标，cls/sum均差`0.052`；DetA/AssA为`45.600/68.650`与`54.623/74.123`，pair mAP/AP50为`0.3207/0.5306`。继续且仅继续e68/e72。
...
## 2026-08-13 13:35 CST：197/252 e40完整闭环

- 197 `0813_01` e40同一checkpoint cls HOTA/DetA/AssA为`54.242/45.369/66.364`，det为`60.673/52.850/71.969`，sum `114.915`，pair mAP/AP50为`0.3046/0.5173`。相对e36提升`1.519/1.074`，sum提高`2.593`。419,316,519-byte checkpoint、5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和345.2秒异步评测完整；训练在GPU4/5继续e41且日志有限。
- 固定252 `0812_05` e40同一checkpoint cls HOTA/DetA/AssA为`54.453/46.218/65.614`，det为`61.070/53.534/72.106`，sum `115.523`，pair mAP/AP50为`0.3105/0.5258`。相对e36提升`1.869/0.768`，sum提高`2.637`。419,504,118-byte checkpoint、5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和451.7秒异步评测完整；训练仍固定GPU0/1继续e41，GPU2的外部任务未触碰。
- 两条尚处于稳定高LR阶段：197的25% floor到e60后才分叉，252的cosine退火到e48后才开始。e40的共同上升否定了按e36平台提前筛掉252的做法，继续闭环后续成熟节点。

## 2026-08-13 14:40 CST：99/178 e60完整闭环

- 99 `0812_03` e60同一checkpoint cls HOTA/DetA/AssA为`55.062/46.040/67.707`，det为`62.111/54.362/73.394`，sum `117.173`，pair mAP/AP50为`0.3108/0.5281`。相对e56为`-0.006/+0.077`，严格目标差距为`0.201/0.488/0.689`。e60 checkpoint约427 MiB，5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和308.8秒异步评测完整；训练继续e61且日志有限。非零floor抑制了明显回落，但目前提升强度不足，仍保留到e72终判。
- 178 `0812_04` e60同一checkpoint cls HOTA/DetA/AssA为`55.807/46.222/69.746`，det为`62.290/54.610/73.447`，sum `118.097`，pair mAP/AP50为`0.3210/0.5390`。相对e56双升`0.322/0.338`；cls和sum已分别超过目标`0.544/0.235`，det仅差`0.309`。e60 checkpoint约427 MiB，5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和310.2秒异步评测完整；训练继续e61且日志有限。
- 178标准WSD在e48→e52→e56→e60连续退火节点上保持改善，已成为最有希望的正式线。目标明确要求e72同点达标，故继续收集e64/e68/e72，不以e60代替终判。

## 2026-08-13 15:05 CST：197 e44完整闭环；252 e44后处理

- 197 `0813_01` e44同一checkpoint cls HOTA/DetA/AssA为`54.747/45.764/67.014`，det为`61.259/53.440/72.577`，sum `116.006`，pair mAP/AP50为`0.3085/0.5221`。相对e40提高`0.505/0.586`。424,780,071-byte checkpoint、5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和433.5秒异步评测完整；训练在GPU4/5继续e45且日志有限。
- 固定252已生成424 MiB级e44 checkpoint并完成5416/50检测，异步轨迹生成于14:53启动。进程持续高CPU并顺序读取/data4检测文件，累计读取超过139 MB；尚未完成TrackEval，故不提前登记e44性能。训练已在固定GPU0/1继续e45。

## 2026-08-13 15:17 CST：固定252 e44完整闭环
...
## 2026-08-13 16:03 CST：197 floor-25 WSD e48完整闭环

- 197 `0813_01` e48同一checkpoint cls HOTA/DetA/AssA为`54.455/45.898/66.111`，det为`61.367/53.718/72.463`，sum `115.822`，pair mAP/AP50为`0.3111/0.5230`。相对e44，cls回落`0.292`而det提高`0.108`，仍处于e56前稳定高LR段；25% floor要到e60后才与零floor分叉，因此不作提前否决。
- `epoch_48.pth`为`430,238,183` bytes；检测为`5,416/50`、51文件，TrackEval为28 CSV/108文件，异步评测耗时356.9秒。GPU4/5正式训练已继续到e49且total/DN/encoder loss与grad norm有限，无traceback/OOM/NCCL异常。

## 2026-08-13 16:40 CST：固定252 smooth-WSD e48完整闭环
...
## 2026-08-13 18:58 CST：197/e56与252/e52完整闭环
...
## 2026-08-13 19:35 CST：197/e60与252/e56收尾节点
...
- 197 `0813_01` e36同一checkpoint cls HOTA/DetA/AssA为`52.723/43.544/66.080`，det为`59.599/51.691/71.081`，sum `112.322`，pair mAP/AP50为`0.2889/0.4900`。
- 相对e32，cls/det HOTA提高`0.800/0.467`，sum提高`1.267`。413,852,839-byte checkpoint、5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和348.7秒异步评测完整；训练在GPU4/5恢复到e37i400，日志与梯度有限。
- 25% floor仅在e60后与零floor轨迹分叉；e36证明fresh轨迹仍健康上升，但不构成对floor干预的早期结论，继续到e72。

## 2026-08-13 12:12 CST：固定252 smooth-WSD e36完整闭环

- 252 `0812_05` e36同一checkpoint cls HOTA/DetA/AssA为`52.584/44.219/64.082`，det为`60.302/52.201/72.158`，sum `112.886`，pair mAP/AP50为`0.2949/0.5004`。
- 相对e32，cls/det HOTA分别提高`0.074/0.129`，sum提高`0.203`。414,020,342-byte checkpoint、5416/50检测、51个检测文件、28 CSV/108 TrackEval文件和379.6秒异步评测完整；训练已在固定GPU0/1恢复到e37且日志有限。
- 本线到e48后才开始cosine退火，e36增幅收窄但未反转，不作为否决依据；继续到e40/e44/e48及实际退火节点。
...
## 2026-08-13 09:33 CST: 252 e28 and 99/178 e44 complete

- Fixed-252 e28 cls HOTA/DetA/AssA is `51.270/43.535/61.823`; det is
  `59.249/51.414/70.672`, sum `110.519`, and pair mAP/AP50 is `0.2857/0.4901`.
  Relative to e24, HOTA rises by `0.564/0.756`, sum by `1.320`; its 403,023,926-byte checkpoint,
  detection and 388.8-second TrackEval are complete.
- 99 floor-cosine e44 is `54.902/61.655`, sum `116.557`, with cls DetA/AssA
  `45.841/67.584`, det `54.134/72.639`, pair `0.3113/0.5288`. It improves e40 by
  `0.420/0.149` and is short of the final target by `0.361/0.944/1.305`.
- 178 smooth-WSD e44 is `54.369/61.345`, sum `115.714`, with cls `45.100/68.013`, det
  `53.841/72.353`, pair `0.3114/0.5256`. It improves e40 by `0.132/0.113`.
...
- 252 `0812_05` e16同一checkpoint的cls HOTA/DetA/AssA为`48.584/39.790/61.347`，det为
  `56.750/49.267/67.676`，sum `105.334`；相对e12双HOTA提高`1.379/3.581`、sum提高
  `4.960`，pair mAP/AP50也由`0.2460/0.4297`升至`0.2561/0.4444`。
- e16 checkpoint `386,536,438` bytes、5416/50检测和异步TrackEval完整，评测耗时
  `373.8s`；训练已恢复e17i200，GPU0/1各约21.4 GiB，GPU2/3保持空闲。252是最慢资源，
  当前连续增长，不用e16提前换线，继续e20及后续成熟节点。
- 同时99与178均到e32训练阶段；197已生成e12 checkpoint并完成
  5416/50检测，异步TrackEval运行中且训练恢复e13。四条正式日志的总损失、DN、encoder项与
  梯度均有限，资源分配仍为99 GPU0/2、178 GPU0、197 GPU4/5、252 GPU0/1。

## 2026-08-13 05:21 CST：197 floor-25 WSD e12完整闭环

- 197 `0813_01` e12同一checkpoint的cls HOTA/DetA/AssA为`46.128/37.797/59.120`，det为
  `53.493/47.492/62.448`，sum `99.621`；相对e8双HOTA提高`3.901/6.066`，pair
  mAP/AP50由`0.2152/0.3828`升至`0.2384/0.4156`。
- e12 checkpoint `380,990,951` bytes、5416/50检测和异步TrackEval完整，评测耗时
  `339.7s`；训练恢复e13i250，正式日志有限。25% floor只在e60后的余弦尾段与零floor WSD
...
## 2026-08-13 01:52 CST：197短尾WSD e72失败并换为25% floor正式线
...
## 2026-08-13 02:59 CST：99 floor-cosine e24完整闭环并继续

- 99 e24同一checkpoint的cls HOTA/DetA/AssA为`52.748/44.199/64.972`，det为
  `59.179/52.068/69.589`，sum `111.927`；相对e20双HOTA提高`0.757/0.347`、sum提高
  `1.104`，当前仍在较高LR阶段，不能代表floor-cosine的最终尾部效果。
- pair mAP/AP50为`0.2926/0.5016`，both-independent为`0.3363/0.5498`；checkpoint
  `397,531,126` bytes，5416条/50序列检测、28 CSV、108个非空TrackEval文件、50条预测及
  `async_done=1`齐全。训练已恢复e25i300，继续成熟节点而不提前换线。
- 178到e24i800；197 e4 checkpoint `369,971,559` bytes已生成并等待完整评测；252到e10i500。
...
- 197 e56同一checkpoint为cls HOTA/DetA/AssA `53.993/44.659/66.949`，det为`61.814/53.922/73.136`，sum `115.807`；pair mAP/AP50 `0.3110/0.5241`，both-independent `0.3469/0.5564`。
- e52→e56的sum仅`-0.002`。cls DetA下降`0.274`而AssA上升`0.122`，det DetA/AssA分别上升`0.033/0.038`，说明平台主要来自cls定位回落，不是关联崩坏。5416/50检测、28 CSV、108非空评测文件、50预测和`async_done=1.0`齐全；训练已恢复e57i850，继续e60及退火后的e64/e72。
- 99 e4为`32.411/39.594`，178 e4为`35.363/42.179`，两者均完成checkpoint、5416/50检测和异步TrackEval并已进入e5。e4仅验证warmup和运行链路，不用于否决任何decoder或调度，继续e12及成熟节点。
...
- e40同点cls HOTA/DetA/AssA `53.109/43.911/65.935`，det `60.938/52.981/72.528`，sum `114.047`；pair mAP/AP50 `0.2992/0.5055`，both-independent `0.3407/0.5482`。
- 5416/50检测、28 CSV、108非空评测文件、50预测和`async_done=1.0`齐全。checkpoint为419,229,735字节，meta `40/41520`、model/EMA 711/712 states，SHA-256 `24e20316824769497a9bcb592f794f5db784028e723d846a5e2495e579ea2835`，642个浮点tensor全有限。
- 当前GPU4/5已健康恢复e41i500，稳定LR `1.5e-4`；e40只是WSD较早成熟节点，不作为decoder否决依据，继续e44/e72。AutoDL正在训练e64，作为近期下一闭环。

## 2026-08-12 16:13 CST：197新增双卡在GPU4/5落实，e40检测完成

- `0812_01`真实占用197 GPU4/5，双rank、正式日志和数值均健康；e40 checkpoint已生成，5416条/50序列检测完成。
- e40 pair mAP/AP50为`0.2992/0.5055`，both-independent为`0.3407/0.5482`；异步TrackEval已启动但尚未完成，因此暂不填写HOTA或宣称e40闭环。
- AutoDL同期到e63 iter500，LR约`1.4003e-5`且训练有限，继续收费主线至e64/e72；197则继续标准WSD至e72。

## 2026-08-12 15:38 CST：AutoDL e60完整闭环并继续e64/e72

- e60同一checkpoint的cls HOTA/DetA/AssA为`54.574/44.138/70.416`，det为
  `62.387/54.595/73.734`，sum `116.961`；相对e56 HOTA为`-0.188/-0.178`，
  距目标`0.689/0.212/0.901`，严格失败但不是目标终点。
- pair mAP/AP50为`0.3141/0.5179`，both-independent为`0.3507/0.5508`；5416条、
  50序列检测、28 CSV、108非空文件、50条非空轨迹和`async_done=1.0`完整。AP和DetA
  同步回撤，说明e56→e60不仅是关联波动。
- checkpoint为446,956,340字节，SHA-256
  `7054d214b9f08d6d3299368676754aadb759ec0373b40cc3ddbfd4d6c88f834f`，642个浮点
  checkpoint tensor全有限。训练已恢复e61 iter250，finalizer存活，继续e64/e72。
...
- e48同一checkpoint的cls HOTA/DetA/AssA为`54.833/44.783/69.907`，det为
  `62.278/54.658/73.407`，sum `117.111`；cls/det MOTA分别`46.752/62.562`，
  IDF1分别`63.952/73.271`。pair mAP/AP50为`0.3153/0.5230`，
  both-independent为`0.3532/0.5576`。5416条、50序列检测与50/50轨迹完整，
  28个CSV、108个非空评测文件、`track/async_done=1.0`，TrackEval自然耗时166.7秒。
- `epoch_48.pth`为430,449,716字节，SHA-256
  `f3b097eea376e6e2fcf796710273c9c991cad1905caae8f898427a57dc1c507f`，meta为
  `epoch=48/iter=49824`；model/EMA为711/712 states，递归审计到的2,776个浮点
  checkpoint tensor全部有限。
- 相对自身e44，cls/det HOTA双升`0.421/0.128`、联合升`0.549`；cls
  DetA/AssA升`0.152/0.964`，det DetA升`0.271`而AssA微降`0.094`，pair
  mAP/AP50升`0.0015/0.0011`、both-independent mAP升`0.0011`而AP50降`0.0008`。
  相对warmup12 e48 `54.816/61.976`，HOTA首次同点双升`0.017/0.302`、联合高
  `0.319`，但四项AP仍低。距最终cls/det/sum目标缩至`0.430/0.321/0.751`；训练已
  自然恢复到e49 iter250，数据盘余约7.0 GiB，继续e52及后续成熟节点。
...
- e40同一checkpoint的cls HOTA/DetA/AssA为`54.506/44.894/68.723`，det为
  `61.996/54.467/73.065`，sum `116.502`；cls/det MOTA分别`47.426/61.950`，
  IDF1分别`63.599/72.707`。pair mAP/AP50为`0.3153/0.5269`，
  both-independent为`0.3543/0.5643`。5416条、50序列检测与50/50轨迹完整，
  28个CSV、108个非空评测文件、`track/async_done=1.0`，TrackEval自然耗时166.3秒。
- `epoch_40.pth`为419,474,932字节，SHA-256
  `ebe35f2c4634049e64f08f864daa2d896104aba9765aa6ac8fbe4c770f22ccf3`，meta为
  `epoch=40/iter=41520`；model/EMA为711/712 states，递归审计到的2,776个浮点
  checkpoint tensor全部有限。
- 相对自身e36，cls/det HOTA升`1.851/0.360`、联合升`2.211`；cls DetA/AssA升
  `1.663/1.281`，det DetA升`0.644`、AssA微降`0.100`，四项AP升约
  `0.0123/0.0192/0.0126/0.0182`。相对warmup12 e40 `54.541/61.711`，HOTA为
  `-0.035/+0.285`、联合高`0.250`；cls AssA高`1.727`、det DetA高`0.667`，但
  AP50仍低。距最终cls/det/sum目标为`0.757/0.603/1.360`，训练已自然恢复到e41，
  继续e44及后续成熟节点，不以e40未达最终目标否决。

## 2026-08-12 09:55 CST：AutoDL e36完整闭环，e32回撤恢复且联合优势保持

- e36同一checkpoint的cls HOTA/DetA/AssA为`52.655/43.231/67.442`，det为
  `61.636/53.823/73.165`，sum `114.291`；cls/det MOTA分别`45.983/60.814`，
  IDF1分别`61.023/72.229`。pair mAP/AP50为`0.302992/0.507713`，
  both-independent为`0.341717/0.546061`。5416条、50序列检测与50/50轨迹完整，
  28个CSV、108个非空评测文件、`track/async_done=1.0`，TrackEval自然耗时165.8秒。
- `epoch_36.pth`为413,988,020字节，SHA-256
  `b1966b8fbece648df39bf5b22add76ce1f0ee4aa8107d4fedd8c6f4f80ad9325`，meta为
  `epoch=36/iter=37368`；model/EMA为711/712 states，递归审计到的2,776个浮点
  checkpoint tensor全部有限。
- 相对自身e32，cls/det HOTA双升`0.439/0.437`、联合升`0.876`；cls
  DetA/AssA升`0.439/0.206`，det升`0.398/0.484`，pair mAP/AP50升
  `0.002063/0.002912`、both-independent升`0.001804/0.001786`，e32阶段性回撤已恢复。
  相对warmup12 e36，HOTA为`-0.253/+0.754`、联合高`0.501`；cls AssA高`1.741`，
  det DetA/AssA高`1.138/0.264`，但cls DetA低`0.703`且AP50仍低。相对原始父线e36
  `53.326/59.293`为`-0.671/+2.343`、联合高`1.672`。距e72目标仍为
  `2.608/0.963/3.571`，训练已自然恢复到e37 iter550，继续e40及后续成熟节点。

## 2026-08-12 08:55 CST：AutoDL e32完整闭环，det继续成熟但cls与AP阶段性回撤

- e32同一checkpoint的cls HOTA/DetA/AssA为`52.216/42.792/67.236`，det为
  `61.199/53.425/72.681`，sum `113.415`；cls/det MOTA分别`45.159/59.912`，
  IDF1分别`60.541/71.686`。pair mAP/AP50为`0.300928/0.504801`，
  both-independent为`0.339913/0.544275`。5416条、50序列检测与50/50轨迹完整，
  28个CSV、108个非空评测文件、`track/async_done=1.0`，TrackEval自然耗时167.8秒。
- `epoch_32.pth`为408,488,628字节，SHA-256
  `65044958b63f3961be0329fb47421cb7ebd77032dc8399cc4fb6e64b7857db04`，meta为
  `epoch=32/iter=33216`；model/EMA为711/712 states，递归审计到的2,776个浮点
  checkpoint tensor全部有限。
- 相对自身e28，cls/det HOTA为`-0.169/+0.592`、联合仍升`0.423`；cls
  DetA/AssA为`-0.386/+0.686`，det为`+0.245/+1.118`，而pair mAP/AP50下降
  `0.003944/0.005371`、both-independent下降`0.004628/0.006175`。相对warmup12
  e32，HOTA为`-0.381/+0.884`、联合高`0.503`；cls DetA低`1.296`但AssA高
  `2.842`，det DetA/AssA高`1.061/0.757`。相对原始父线e32 `53.309/59.320`
  为`-1.093/+1.879`、联合高`0.786`。当前收益仍主要来自关联与det，cls定位及高阈值
  AP出现阶段性回撤，故继续e36及后续成熟节点，不以e32单点停线。训练已自然恢复到
  e33 iter450，GPU0约31.4 GiB、数据盘余约9.7 GiB、明确fatal扫描为0。
...
- e24同一checkpoint的cls HOTA/DetA/AssA为`52.115/43.151/65.666`，det为
  `60.061/52.801/70.770`，sum `112.176`；cls/det MOTA分别`45.007/58.689`，
  IDF1分别`60.594/70.051`。pair mAP/AP50为`0.302287/0.506918`，
  both-independent为`0.342950/0.549071`。5416条、50序列检测与50/50轨迹完整，
  28个CSV、108个非空评测文件、`track/async_done=1.0`，TrackEval自然耗时169.1秒。
- `epoch_24.pth`为397,515,124字节，SHA-256
  `e71d3f7b896b4184d1b5201513c499a174ecac1012a04663cdfd9ce01725b2f7`，meta为
  `epoch=24/iter=24912`；model/EMA为711/712 states，递归审计到的2,776个浮点
  checkpoint tensor全部有限。
- 相对`0810_08`标准warmup12+cosine同点，cls/det HOTA提高`0.265/1.062`、联合
  提高`1.327`；cls DetA下降`0.769`但AssA提高`2.800`，det DetA/AssA提高
  `1.059/1.142`。pair mAP提高`0.008934`而AP50下降`0.001882`，both-independent
  mAP提高`0.005854`而AP50下降`0.008965`。相对原始product-tangent父线e24
  `52.478/58.771`为`-0.363/+1.290`、联合仍高`0.927`；关联优势保持但cls定位与高阈值
  AP仍需后期成熟。训练自然恢复到e25 iter450，继续e28及后续成熟节点，不在e24停线。

## 2026-08-12 06:02 CST：AutoDL e20完整闭环，标准短warmup保持联合优势

- e20同一checkpoint的cls HOTA/DetA/AssA为`51.772/42.780/65.435`，det为
  `59.525/52.529/69.866`，sum `111.297`；cls/det MOTA分别`44.443/58.331`，
  IDF1分别`60.130/69.346`。pair mAP/AP50为`0.299258/0.503352`，
  both-independent为`0.340627/0.546798`。5416条、50序列检测与50/50轨迹完整，
  `track/async_done=1.0`，TrackEval自然耗时166.9秒。
- `epoch_20.pth`为392,033,012字节，SHA-256
  `e2c0d417c058ae7c0e7558448d823d6552e96a17f8a3261eca48c6b02942636f`，meta为
  `epoch=20/iter=20760`；model/EMA为711/712 states，optimizer为497 states/groups，
  递归审计到的2,776个浮点checkpoint tensor全部有限。
- 相对`0810_08`标准warmup12+cosine同点，cls/det HOTA提高`0.544/1.489`；
  cls DetA下降`0.254`但AssA提高`2.627`，det DetA/AssA提高`1.485/1.486`；pair
  mAP/AP50提高`0.010841/0.003201`，both-independent mAP提高`0.008914`而AP50下降
  `0.003165`。相对原始product-tangent父线e20 `52.198/58.132`为
  `-0.426/+1.393`、联合仍高`0.967`。训练自然进入e21，数据盘仍余约12 GiB；继续e24及
  后续成熟节点，不因单点cls回撤提前否决。

## 2026-08-12 05:08 CST：AutoDL e16完整闭环，成熟前移优势仍双正

- e16同一checkpoint的cls HOTA/DetA/AssA为`51.488/42.891/64.243`，det为
  `58.779/52.124/68.640`，sum `110.267`；cls/det MOTA分别`44.638/57.802`，
  IDF1分别`60.145/68.186`。pair mAP/AP50为`0.296979/0.499316`，
  both-independent为`0.340233/0.546737`。5416条、50序列检测与50/50轨迹完整，
  `track/async_done=1.0`，TrackEval自然耗时167.2秒。
- `epoch_16.pth`为386,538,932字节，SHA-256
  `9a4528a18610c9b784496ebc0aa188dbc82f568edcb04ba7192d1b26eec55571`，meta为
  `epoch=16/iter=16608`；model/EMA为711/712 states，optimizer为497 states/groups，
  1,284个浮点model/EMA tensor以及递归审计到的全部2,776个浮点checkpoint tensor均有限。
- 相对`0810_08`标准warmup12+cosine同点，cls/det HOTA提高`2.161/2.082`，
  cls DetA/AssA提高`1.164/3.922`，det提高`1.984/2.205`；pair mAP/AP50提高
  `0.020010/0.018812`，both-independent提高`0.019256/0.013482`。相对原始
  product-tangent父线e16 `51.220/58.370`也双升`0.268/1.409`。训练已自然进入e17，
  数据盘仍余约12.3 GiB；继续e20及成熟节点，不以e16未达最终阈值提前否决。
...
- SSH端点已更新并验证，远端hostname严格匹配实例ID `c12c46bdd8-77ce297d`。实例提供
  1张RTX 5090（32607 MiB），检查时0 MiB/0%、无计算进程；208个CPU线程、754 GiB内存。
- 系统盘约29 GiB可用，数据盘 `/root/autodl-tmp` 约15 GiB可用，共享盘约157 GiB可用；
  `/root/PairMOT/ai4rs`、`/root/autodl-tmp/data/hsmot/{train,test}` 和真实GMC缓存目录存在。
  镜像自带PyTorch为2.8.0+cu128，CUDA可用且识别1卡。
- 按用户明确要求，不运行PairMOT smoke、模型构建、真实数据加载或formal训练；状态只登记为
  `ONLINE/RESOURCE_CHECKED/IDLE/NO_SMOKE/NO_FORMAL`。该实例不能原样执行2卡协议，任何
  1x8等批量适配均须另行决定后再验证。

## 2026-08-12 00:50 CST：197退出本轮资源，三条远端训练线停止写日志

- 普通SSH在约24秒后可进入197；此前失败是过短 `ServerAliveInterval/CountMax` 在服务器
  确认session channel前由客户端主动断开，并非 `/data4` 权限或NFS不可访问。197上
  `/data4` 实际挂载为 `10.106.15.88:/datav2`，容量36T、使用40%。
- 经197读取共享盘并在00:17--00:18间隔35秒复核：99 `0810_08` 停在
  e71i150（日志16:05:24、last checkpoint e68），252 `0810_09` 停在e39i850
  （16:05:22、last checkpoint e36），178 `0811_02` 停在e2i100（16:05:43、尚无
  周期checkpoint）。三份日志大小和末行均不变，精确fatal扫描无Traceback/OOM/NCCL错误；
  只能判定训练未推进，控制面不可达时不能区分退出与阻塞。
- 197主机本身SSH、共享盘、代码与Conda路径可见，6张RTX 3090连续空闲且无既有训练进程；
  但py310环境的 `import torch` 在60秒内未完成，双卡NCCL诊断也未返回。按用户决定放弃在
  197运行实验。遗留诊断PGID `3843487` 已精确TERM，临时文件已由trap移除，测试进程归零，
  6卡最终均为1 MiB/0%；没有创建smoke/formal workdir或修改实验仓库。
...
- 178 `0810_06` One-Cycle e72 为 cls HOTA/DetA/AssA
  `54.162/44.589/68.101`、det `62.280/54.464/73.498`，sum `116.442`。相对 e68，
  HOTA `-0.296/+0.062`、sum `-0.234`；cls DetA/AssA `-0.268/-0.358`，det
  `-0.079/+0.259`，四项 AP 全降至 pair `0.310752/0.512149`、both-independent
  `0.347665/0.545261`。距目标 cls/det/sum `1.101/0.319/1.420`，标准 One-Cycle
  最终严格失败。463,830,388-byte checkpoint SHA-256
  `5b499149c035d5029d02a8721d0c4723495b7cc0f1ca3380da8913914bbd353a`，meta
  `72/74736`，model/EMA 711/712、optimizer 497 states/groups、1 scheduler、
  2,776 浮点 tensor 全有限；5416/50、51/28/108/50 与 async=1 全闭环，训练自然退出。
- 99 `0810_08` cosine e68 为 cls `54.387/45.032/67.653`、det
  `62.298/54.093/74.181`，sum `116.685`。相对 e64，HOTA
  `+0.033/+0.099`、sum `+0.132`；cls DetA/AssA `-0.115/+0.343`，det
  `-0.006/+0.229`，但四项 AP 继续下降至 pair `0.309780/0.526247`、
  both-independent `0.348879/0.563237`，e52 仍为本线与总榜最佳。457,870,070-byte
  checkpoint SHA-256 `d55951a2fcc804a43f25e2e8c31c3c51bf789f904fc27e4399ebb500bc90f22b`，
  meta `68/70584`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor
  全有限；5416/50、51/28/108/50 与 async=1 全闭环，继续 e72。
- 252 `0810_09` WSD e36 为 cls `52.478/43.485/65.096`、det
  `60.531/52.523/72.206`，sum `113.009`。相对 e32，HOTA `+0.088/+0.268`、
  sum `+0.356`；cls DetA/AssA `+0.072/-0.132`，det `+0.200/+0.338`，四项 AP
  全升至 pair `0.294413/0.496056`、both-independent `0.336403/0.539560`。
  413,798,326-byte checkpoint SHA-256
  `b4a5c8354c365f2ba02b1062a3bfe43ae80d8b46add96501034530d36e21cde7`，meta
  `36/37368`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51/28/108/50 与 async=1 全闭环。距目标 `2.785/2.068/4.853`，增长放缓但
  未反转，继续 e40/e72。
- 实时 99 e69i600、252 e38i500，日志有限且资源正确；178 训练自然退出，GPU0 空闲，GPU1
  外部 InternVL 不动。下一候选优先把静态验证的标准 warmup4-cosine 适配为 178 `1x8`，
  保持全局 batch 8，并须通过五项动态门槛后才登记 RUNNING。

## 2026-08-11 14:21 CST：cosine e64 平台，One-Cycle e68 小幅恢复但未创新高

- 99 `0810_08` cosine e64 为 cls HOTA/DetA/AssA
  `54.354/45.147/67.310`、det `62.199/54.099/73.952`，sum `116.553`。相对 e60，
  HOTA `+0.117/-0.107`、sum `+0.010`；cls DetA/AssA `+0.110/+0.163`，det
  `+0.035/-0.315`，四项 AP 全降至 pair `0.310301/0.528114`、both-independent
  `0.349620/0.565467`，说明检测关联回撤抵消分类恢复，e52 仍为本线和总榜最佳。
  452,382,198-byte checkpoint SHA-256
  `6aebf315334697c5c757c93e1d5a74c21ad6178d369dcf49b90fc92c1c4454e6`，meta
  `64/66432`，model/EMA 711/712、optimizer 497 states/groups、2 scheduler、
  2,776 浮点 tensor 全有限；5416/50、51/28/108/50 与 async=1 全闭环。
- 178 `0810_06` One-Cycle e68 为 cls `54.458/44.857/68.459`、det
  `62.218/54.543/73.239`，sum `116.676`。相对 e64，HOTA
  `+0.047/+0.024`、sum `+0.071`；cls DetA/AssA `+0.003/+0.110`，det
  `-0.112/+0.210`，但四项 AP 继续下降至 pair `0.311871/0.514154`、
  both-independent `0.349024/0.547445`。相对本线 e60 仍低 `0.163/0.114/0.277`，
  e60 保持本线最佳。458,310,964-byte checkpoint SHA-256
  `35048e05044cf6cc461bc2145886830e981a923be57aa9fd77f3b7dcbd61807a`，meta
  `68/70584`，711/712、497 states/groups、1 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51/28/108/50 与 async=1 全闭环。
- 两线距 e72 目标分别为 cosine e64 `0.909/0.400/1.309`、One-Cycle e68
  `0.805/0.381/1.186`。虽低 LR 尾段没有产生新峰值，但按目标继续到 e72 做同一 checkpoint
  严格判定。实时为 99 e65i500、178 e69i300、252 e35i50，正式日志有限且资源边界正确；
  下一节点优先为 WSD e36，随后两条快线 e72。

## 2026-08-11 13:56 CST：cosine/One-Cycle 低 LR 尾段回撤，WSD e32 继续双升

- 99 `0810_08` cosine e60 为 cls HOTA/DetA/AssA
  `54.237/45.037/67.147`、det `62.306/54.064/74.267`，sum `116.543`。相对 e56，
  HOTA `-0.307/+0.068`、sum `-0.239`；cls DetA/AssA `-0.255/-0.274`，det
  `-0.018/+0.165`，四项 AP 全降至 pair `0.310951/0.530423`、both-independent
  `0.350325/0.567751`，故 e52 仍是本线和当前总榜最佳。446,877,814-byte
  checkpoint SHA-256 `5181ad550a50b1bb4b2b1a39430d56d57b0586ca2358e730cd4579f20ca6c33f`，
  meta `60/62280`，model/EMA 711/712、optimizer 497 states/groups、2 scheduler、
  2,776 浮点 tensor 全有限；5416/50、51/28/108/50 与 async=1 全闭环。
- 178 `0810_06` One-Cycle e64 为 cls `54.411/44.854/68.349`、det
  `62.194/54.655/73.029`，sum `116.605`。相对 e60，HOTA
  `-0.210/-0.138`、sum `-0.348`；DetA `-0.126/-0.047`、AssA
  `-0.364/-0.233`，四项 AP 全降至 pair `0.312445/0.514463`、both-independent
  `0.349941/0.548428`，故 e60 仍为本线最佳。452,783,988-byte checkpoint SHA-256
  `b0c3ce1bd781104ec6bb33431ced93ba3c319e6147cca48bcaf7ef8c00938822`，meta
  `64/66432`，711/712、497 states/groups、1 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51/28/108/50 与 async=1 全闭环。
- 252 `0810_09` WSD e32 为 cls `52.390/43.413/65.228`、det
  `60.263/52.323/71.868`，sum `112.653`。相对 e28，HOTA `+0.200/+0.737`、
  sum `+0.937`；cls DetA/AssA `+0.234/-0.108`，det `+0.345/+1.275`，四项 AP
  全升至 pair `0.291946/0.492856`、both-independent `0.335018/0.538283`。
  408,329,974-byte checkpoint SHA-256
  `2a00dadfc7a9d3532e3e622c97624600f548c1a318b42bcb48970272ab91ed14`，meta
  `32/33216`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51/28/108/50 与 async=1 全闭环。距目标 cls/det/sum
  `2.873/2.336/5.209`，但稳定段仍持续改善，保留到 e36/e72。
- 当前总榜仍为 cosine e52 `116.975`；cosine e56/e60 与 One-Cycle e60/e64 的低 LR
  尾段均出现回撤，但用户目标要求严格核验 e72，故两线继续跑满而不提前终止。实时为 99
  e64i400、178 e68i550、252 e33i800，正式日志有限且资源边界正确；下一节点 e64/e68/e36。

## 2026-08-11 12:30 CST：One-Cycle e60 逼近总榜，cosine e56 回撤，WSD e28 连续增长

- 178 `0810_06` One-Cycle e60 为 cls HOTA/DetA/AssA
  `54.621/44.980/68.713`、det `62.332/54.702/73.262`，sum `116.953`，刷新本线
  最佳并只低 cosine e52 总和 `0.022`。相对 e56，HOTA `+0.255/-0.086`、sum
  `+0.169`；cls DetA/AssA `+0.112/+0.621`，det `-0.029/-0.116`。pair
  mAP/AP50 `0.313151/0.517305`，both-independent `0.351281/0.552379`，两项 mAP
  微升、两项 AP50 微降。距目标 cls/det/sum `0.642/0.267/0.909`。447,260,148-byte
  checkpoint SHA-256 `fea200a4fac3dae7b5278f17de8e4025f60d070b3fb22c01fa6fe9be9a09c43d`，
  meta `60/62280`，model/EMA 711/712、optimizer 497 states/groups、1 scheduler、
  2,776 浮点 tensor 全有限；5416/50、51 检测文件、28 CSV/108 非空/50 predictions、
  async=1 全闭环。
- 99 `0810_08` cosine e56 为 cls `54.544/45.292/67.421`、det
  `62.238/54.082/74.102`，sum `116.782`。相对 e52，HOTA
  `-0.265/+0.072`、sum `-0.193`；cls DetA/AssA `-0.207/-0.507`，det
  `+0.029/+0.101`，四项 AP 全降至 pair `0.313985/0.536603`、both-independent
  `0.353235/0.573053`，故 e52 仍是 cosine 与当前总榜最佳。441,382,582-byte
  checkpoint SHA-256 `30cd7a6db3c9bb29dc73ec93a5bb7a32dd4efcfc44930202417e982dbff8293b`，
  meta `56/58128`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor
  全有限；5416/50、51/28/108/50 与 async=1 全闭环，继续 e60/e72。
- 252 `0810_09` WSD e28 为 cls `52.190/43.179/65.336`、det
  `59.526/51.978/70.593`，sum `111.716`。相对 e24，HOTA `+1.056/+0.834`、
  DetA `+0.471/+0.348`、AssA `+1.780/+1.541`，四项 AP 全升至 pair
  `0.290251/0.491216`、both-independent `0.333728/0.537733`。402,860,662-byte
  checkpoint SHA-256 `5b880aa0a4c7e94dcfffd88ea1376998b1637d612bfce8ff200cf39f9b525d56`，
  meta `28/29064`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor
  全有限；5416/50、51/28/108/50 与 async=1 全闭环，继续 e32/e72。
- 总榜仍为 cosine e52 `116.975`，One-Cycle e60 仅低 `0.022`；两线分别需要改善
  `0.454/0.433/0.887` 与 `0.642/0.267/0.909`，无全面支配。实时为 99 e59i750、
  178 e64i50、252 e29i950，正式日志有限且资源边界正确；下一节点 e60/e64/e32。

## 2026-08-11 11:00 CST：cosine e52 再领先，One-Cycle e56 恢复，WSD e24 强增长

- 99 `0810_08` warmup12-cosine e52 为 cls HOTA/DetA/AssA
  `54.809/45.499/67.928`、det `62.166/54.053/74.001`，sum `116.975`，刷新当前
  最高总和。相对 e48，HOTA `-0.007/+0.190`、sum `+0.183`；det AssA `+0.395`，
  但四项 AP 回落至 pair `0.315777/0.540024`、both-independent
  `0.354497/0.575993`。距目标 cls/det/sum `0.454/0.433/0.887`。435,888,374-byte
  checkpoint SHA-256 `64ce252a3e60f59e058380cc1037e3324539bdfb549e01c690023e73e8681204`，
  meta `52/53976`，model/EMA 711/712、optimizer 497 states/497 groups、2 scheduler、
  2,776 浮点 tensor 全有限；5416/50、51 检测文件、28 CSV/108 非空/50 predictions、
  async=1 全闭环。
- 178 `0810_06` One-Cycle e56 为 cls `54.366/44.868/68.092`、det
  `62.418/54.731/73.378`，sum `116.784`，较 e52 HOTA `+0.137/+0.207`、sum
  `+0.344`，并较旧最佳 e48 总和高 `0.195`。cls DetA/AssA `-0.033/+0.302`，det
  `+0.023/+0.414`；四项 AP 中三项微升，both AP50 微降至 pair
  `0.312506/0.518104`、both-independent `0.350893/0.553241`。距目标
  `0.897/0.181/1.078`。441,742,900-byte checkpoint SHA-256
  `ed2dad5246810401c22d149775a5c86426660867f5cb844131754da9a7b64743`，meta
  `56/58128`，711/712、497 states/groups、1 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51/28/108/50 与 async=1 全闭环。
- 252 `0810_09` WSD e24 为 cls `51.134/42.708/63.556`、det
  `58.692/51.630/69.052`，sum `109.826`。相对 e20，HOTA `+1.961/+1.314`、
  DetA `+2.277/+1.551`、AssA `+0.666/+1.073`，四项 AP 大幅增至 pair
  `0.286376/0.486337`、both-independent `0.330717/0.534863`。397,391,798-byte
  checkpoint SHA-256 `e812198a0a7b70ed51d10418faace36e23a18800e7d7a64c7ddb3ff088d0cb45`，
  meta `24/24912`，711/712、497 states/groups、2 scheduler、2,776 浮点 tensor
  全有限；5416/50、51/28/108/50 与 async=1 全闭环，继续 e28/e72。
- 99 e52 相对 178 e56 的 cls/det/sum 为 `+0.443/-0.252/+0.191`，两条依旧互补；
  cosine 总和与 cls 领先，One-Cycle det 更接近门槛。实时为 99 e54i850、178 e59i250、
  252 e25i800，正式日志有限且资源边界正确；99 GPU1 与 178 GPU1 分别为外部 UNet/InternVL，
  PairMOT 未占用。下一节点 e56/e60/e28，`0811_01` 保持静态后备。
...
- 178 `0810_06` One-Cycle e48 为 cls HOTA/DetA/AssA `54.413/45.115/67.809`、
  det `62.176/54.585/73.086`，sum `116.589`。相对 e44，HOTA `+0.117/+0.371`，
  cls/det AssA `+0.501/+0.652`，但四项 AP 小幅回落；pair mAP/AP50
  `0.313493/0.522858`，both-independent `0.353039/0.560230`。距目标
  `0.850/0.423/1.273`。430,718,324-byte checkpoint SHA-256
  `6bde0c1538ef324d75373d7ac1719320943919f00d9454dd65411c669170a131`，meta
  `48/49824`，711/712 states、497 参数组、1 scheduler、2,776 浮点 tensor 全有限；
  5416/50、51 检测文件、28 CSV/108 非空/50 predictions、async=1 全闭环。
- 99 `0810_08` warmup12-cosine e44 为 cls `54.592/45.838/66.764`、det
  `61.828/53.997/73.297`，sum `116.420`。相对 e40，HOTA `+0.051/+0.117`、
  DetA `+0.190/+0.197`，AssA `-0.232/-0.031`，四项 AP 继续上升；pair
  `0.315990/0.544399`，both-independent `0.356856/0.584397`。距目标
  `0.671/0.771/1.442`。424,915,510-byte checkpoint SHA-256
  `23d80dacc2c5e71e6015d367389cf7cf761d453af27789fd36009f113cc7ea77`，meta
  `44/45672`，711/712 states、497 参数组、2 schedulers、2,776 浮点 tensor 全有限；
  检测和 TrackEval 全量闭环。
- 252 `0810_09` WSD e16 为 cls `48.909/40.235/61.799`、det
  `56.825/50.063/66.663`，sum `105.734`。相对 e12，HOTA `+3.070/+3.627`、
  DetA `+2.491/+2.337`、AssA `+3.766/+5.341` 且四项 AP 全升；pair
  `0.265075/0.453282`，both-independent `0.307279/0.500349`。386,449,910-byte
  checkpoint SHA-256 `872afb3dcc4cc87a8d021d76301b6a012f109a49596e29bacd8e0d64f3c1b160`，
  meta `16/16608`，711/712 states、497 参数组、2 schedulers、2,776 浮点 tensor
  全有限；评测全闭环。
- 178 e48 相对 99 e44 的 cls/det/sum 为 `-0.179/+0.348/+0.169`，总和最高但
  仍不全面支配；两条成熟路线继续。实时为 99 e45i350、178 e49i850、252 e17i800，
  数值和 GPU 边界正常；197 正确身份仍超时，未部署后备。下一节点 e48/e52/e20。
...
- 99 `0810_08` warmup12-cosine e40 为 cls HOTA/DetA/AssA
  `54.541/45.648/66.996`、det `61.711/53.800/73.328`，sum `116.252`；pair
  mAP/AP50 `0.313022/0.541495`，both-independent `0.354448/0.583415`。相对 e36，
  HOTA `+1.633/+0.829`，六项 DetA/AssA 与四项 AP 全升；距最终 cls/det/sum
  门槛 `0.722/0.888/1.610`。419,439,798-byte checkpoint SHA-256
  `d417c38041f4bc7d27819f325665366fdab32656b02f2d18180889dcb32e75fe`，meta
  `40/41520`，711/712 states、497 参数组、2 个 scheduler 与 2,776 个浮点 tensor
  全部完整且有限；5416/50、28 CSV/108 非空文件/50 predictions 与 async 全闭环。
- 178 `0810_06` ratio-preserving One-Cycle e44 为 cls `54.296/45.257/67.308`、det
  `61.805/54.445/72.434`，sum `116.101`；pair `0.315597/0.530427`，
  both-independent `0.355256/0.568277`。相对 e40，HOTA `+0.294/+0.036`、DetA
  `+0.472/+0.372`，AssA `-0.136/-0.460`，四项 AP 仍全升；距目标
  `0.967/0.794/1.761`。425,217,396-byte checkpoint SHA-256
  `b9e93b4789553ee1c373ac8bfa3a9f3b9d56551a63023ae03b8138df2f8363e8`，meta
  `44/45672`，711/712 states、497 参数组、1 个 scheduler、2,776 浮点 tensor 全有限，
  5416/50、28/108/50 与 async 全闭环。
- 同成熟窗口比较，99 e40 相对 178 e44 的 cls/det/sum 为
  `+0.245/-0.094/+0.151`；余弦更强的 cls 和总和、One-Cycle 略强的 det 仍互补，且两条
  自身 AP 均增长，因此均继续下一个完整节点与 e72，不作中途停线。
- 252 WSD e12 为 cls `45.839/37.744/58.033`、det `53.198/47.726/61.322`，sum
  `99.037`；pair `0.241154/0.415925`，both-independent `0.283152/0.465866`。
  相对 e8，HOTA `+2.698/+4.643`、DetA `+2.173/+3.791`、AssA
  `+2.936/+5.716`，四项 AP 全升。380,977,590-byte checkpoint SHA-256
  `be2a1a731c388cacd7e4ba8a64fb5ed9125383c64a1294049174671e6cec7479`，meta
...
- 178 `0810_06` ratio-preserving One-Cycle peak×2.5 e40 为 cls
  HOTA/DetA/AssA `54.002/44.785/67.444`、det `61.769/54.073/72.894`，sum
  `115.771`；pair mAP/AP50 `0.311467/0.526878`，both-independent
  `0.352117/0.566599`。相对 e36，cls 三项 `+1.644/+1.649/+1.477`、det 三项
  `+0.532/+0.575/+0.383`，四项 AP `+0.008533/+0.013740/+0.008221/+0.012106`。
  419,711,476-byte checkpoint SHA-256
  `aa15d33e41110897e3c3f9d7b165b7eefc4ddfb1159bf98a5dca629fcbe7a555`，2,776
  浮点 tensor 全有限；5416/50、28/108/50 与 async 全闭环。该点距最终 cls/det/sum
  门槛 `1.261/0.830/2.091`，是当前最强成熟点但尚未达标，继续 e44/e72。
- 99 `0810_08` warmup12-cosine e36 为 cls `52.908/43.934/65.701`、det
  `60.882/52.685/72.901`，sum `113.790`；pair `0.301377/0.523105`，
  both-independent `0.341658/0.564110`。相对 e32，HOTA `+0.311/+0.567`，cls/det
  AssA `+1.307/+0.977`、det DetA `+0.321`；cls DetA `-0.154`，both-independent
  AP50 仅 `-0.000042`，其余 AP 上升。413,966,518-byte checkpoint SHA-256
  `a3d5bdf02f78ed643519db421ab4592f1869a99c34b58e85f7ef3e70ec1a1ee3`，2,776
  浮点 tensor 全有限，5416/50、28/108/50 与 async 全闭环。
- 同 e36，99 余弦的 cls/det/sum 相对 One-Cycle为 `+0.550/-0.355/+0.195`，并有更高
  cls DetA、det AssA 与 AP50；One-Cycle 有更高 cls AssA、det DetA 和 mAP。两者没有
  全面支配，且余弦总和仍升，继续 e40/e72；One-Cycle 继续 e44/e72。
- 252 WSD e8 为 cls `43.141/35.571/55.097`、det `48.555/43.935/55.606`，sum
  `91.696`；pair `0.217427/0.377477`，both-independent `0.260923/0.435138`。相对
  e4，HOTA `+11.037/+8.549`、DetA `+11.422/+12.887`、AssA `+8.989/+3.184`，
  四项 AP 全升。375,508,598-byte checkpoint SHA-256
  `b304c68424b6b9abe266d30f94c7a306cc150a64b86d7baa5f61711aa3fd694a`，2,776
  浮点 tensor 全有限，5416/50、28/108/50 与 async 全闭环。同 e8 比余弦高
  `2.940/1.847`，但只作早期形成证据，不以 e8 判胜负或停线，继续 e12/e72。
- 实时训练为 99 e37i300、178 e41i850、252 e10i1000，正式日志、GPU 与有限性健康；
  下一完整节点为 e40/e44/e12。197 仍不可达，`0811_01` 保持静态后备且不占卡。

## 2026-08-11 04:14 CST：99 e32 与 178 e36 继续成熟增长

- 99 `0810_08` standard warmup12-cosine e32 同一 checkpoint 为 cls
  HOTA/DetA/AssA `52.597/44.088/64.394`、det `60.315/52.364/71.924`，sum
  `112.912`；pair mAP/AP50 `0.297949/0.520409`，both-independent
  `0.339567/0.564152`。相对 e28，cls 三项 `+0.623/+0.216/+0.859`、det 三项
  `+0.597/+0.246/+1.032`，四项 AP 分别 `+0.001785/+0.002947/+0.000641/+0.000784`。
  408,479,606-byte checkpoint SHA-256
  `e576146b404af22881488012a62cdde4dc094fec8df844115fb8a827bd77b71a`；遍历
  checkpoint 的 2,776 个浮点 tensor 全有限，5416/50、28/108/50 与 async 全闭环。
- 178 `0810_06` ratio-preserving standard One-Cycle peak×2.5 e36 为 cls
  `52.358/43.136/65.967`、det `61.237/53.498/72.511`，sum `113.595`；pair
  `0.302935/0.513138`，both-independent `0.343896/0.554494`。相对 e32，cls
  三项 `+0.919/+0.672/+1.158`、det 三项 `+0.387/+0.560/+0.119`，四项 AP
  `+0.007545/+0.012319/+0.007743/+0.011899`，没有峰后回撤。414,205,108-byte
  checkpoint SHA-256 `4a7612ae96506b6e45168066e8bc756065f247bcc9b793cf7e6dff19acc81b10`，
  2,776 浮点 tensor 全有限，5416/50、28/108/50 与 async 全闭环。
- 同 e32 比较，99 余弦的 cls HOTA、sum 与四项 AP 分别领先 `1.158/0.623` 和
  `0.002558/0.019589/0.003414/0.021557`；178 One-Cycle 的 det HOTA 与 cls/det
  AssA 分别领先 `0.535/0.415/0.468`。两条仍互补且各自全指标上升，均继续；当前最强
  成熟点是 One-Cycle e36 的 `113.595`，距目标 cls/det/sum 尚差
  `2.905/1.362/4.267`，不能判定达标。
- 实时训练为 99 e33i350、178 e37i850、252 WSD e7i550，正式日志和有限性健康；252
  继续到 e8，99/178 的下一完整节点为 e36/e40。197 短超时只读 SSH 再次超时，未部署、
  未占卡；`0811_01` 仍是静态后备，不越界启动。

## 2026-08-11 03:37 CST：99 e28、178 e32 与 252 e4 完整闭环

- 99 `0810_08` warmup12-cosine e28 同一 checkpoint 为 cls HOTA/DetA/AssA
  `51.974/43.872/63.535`、det `59.718/52.118/70.892`，sum `111.692`；pair
  mAP/AP50 `0.296164/0.517461`，both-independent `0.338926/0.563368`。相对 e24，
  cls HOTA/DetA/AssA 为 `+0.124/-0.048/+0.669`，det 为 `+0.719/+0.376/+1.264`，
  四项 AP 全升。402,992,438-byte checkpoint SHA-256
  `7bfe550ba525419b779b67f6a50c7abb7f21ad38902649583218cae2890a7841`，浮点
  tensor 全有限；5416/50、28 CSV/108 非空文件/50 predictions、`async_done=1` 全闭环。
- 178 `0810_06` ratio-preserving One-Cycle peak×2.5 e32 为 cls
  `51.439/42.464/64.809`、det `60.850/52.938/72.392`，sum `112.289`；pair
  `0.295390/0.500819`，both-independent `0.336153/0.542595`。相对 e28，cls
  HOTA/DetA/AssA 为 `+0.126/+0.259/-0.117`，det 为 `+0.607/+0.362/+0.983`；
  pair 两项与 both-independent mAP 回升，仅 both-independent AP50 `-0.000166`。
  408,691,956-byte checkpoint SHA-256
  `d296519e67e3ec2a6aa513b960bad9274815d98deb125429f401f04517792a3e`，浮点
  tensor 全有限；检测和 TrackEval 的 5416/50、28/108/50 与 async 门槛全闭环。
- 同 e28 比较，99 的 cls HOTA、sum 分别高 `0.661/0.136`，且四项 AP 更高；178 的
  det HOTA 高 `0.525`，cls/det AssA 更强。两者没有全面支配，且 178 e28→e32 仍双升，
  因此均继续至下一成熟节点和 e72，不作早停；当前 99 e31i350、178 e36i50，fatal=0。
- 252 WSD e4 最终为 cls `32.104/24.149/46.108`、det
  `40.006/31.048/52.422`，sum `72.110`；pair `0.125499/0.234166`、
  both-independent `0.167094/0.302948`。369,969,206-byte checkpoint SHA-256
  `bb7633caaaf4d752cc301de923932c4a7263dc8184e26d46acd152ca846e556c`，遍历
  checkpoint 的 2,776 个浮点 tensor 全有限；5416/50、28 CSV/108 非空文件/50 predictions
  和 `async_done=1` 全闭环。训练独立推进到 e5i750。e4 只是四轮标准 warmup 的诊断点，
  不能据此否决 WSD，继续 e8/e72。197 只读 SSH 再次超时，未改变其代码、workdir 或 GPU 状态。
...
- 178 `0810_06` One-Cycle peak×2.5 的 e28 同一 checkpoint 为 cls HOTA/DetA/AssA
  `51.313/42.205/64.926`、det `60.243/52.576/71.409`，sum `111.556`；pair
  mAP/AP50 `0.292999/0.500325`，both-independent `0.333631/0.542761`。相对自身
  e24，HOTA `+0.233/+0.968`、DetA `+0.104/+0.582`、AssA `+0.643/+1.545`，
  四项 AP 略回撤，不能据单点停止；403,176,116-byte checkpoint SHA-256
  `fe99173216c65067912d3979fe8a426ad014944c9bbe8b39f8d86d05f4de0806`，642 个
  浮点 tensor、iterative-cls/DN、5416/50、28 CSV/108 非空文件/50 predictions 全闭环。
  当前训练到 e30 iter1000，fatal=0，继续 e32/e72。
- 99 `0810_08` warmup12-cosine 的 e24 为 cls `51.850/43.920/62.866`、det
  `58.999/51.742/69.628`，sum `110.849`；pair `0.293353/0.508800`，
  both-independent `0.337096/0.558036`。相对 e20，HOTA `+0.622/+0.963`、
  DetA `+0.886/+0.698`、AssA `+0.058/+1.248`，四项 AP 全升。checkpoint
  397,499,574 bytes，SHA-256
  `c887c28c0c8609cc3e324465a1df498de37f9f65a9dd453c55e95d05f9e9c1ee`，完整性、
  检测与 TrackEval 全闭环；当前 e26 iter400，fatal=0，继续 e28/e72。
- 252 `0810_07` One-Cycle peak×2.0 e24 为 cls `48.913/40.141/61.988`、det
  `55.278/49.174/64.653`，sum `104.191`；pair `0.259727/0.455464`，
  both-independent `0.301252/0.505747`。checkpoint 397,539,638 bytes，SHA-256
  `cb74956db753eeb3047eca9adf0d35bea2933847fc8a0c854b36a39cd4972d4b`，642 tensor、
  iterative-cls/DN、5416/50 与 28/108/50 均闭环。该成熟点的 HOTA、DetA、AssA 与
  AP 均被 178 同点支配，故在 e25 iter1000 精确停止，进程归零、固定 GPU0/1 释放；
  这是峰后成熟否决，不是 e4/e8 直接否决。
- 释放后将标准 WSD `0810_09` 部署到 252 独立 clean checkout，commit `34efcf4`。
  该方案仅将学习率替换为 4 epoch 线性 warmup 到父线 `1.5×`、56 epoch stable、
  12 epoch cosine decay；模型、参数、数据语义、loss、EMA、全局 batch 与推理图不变。
  远端配置 deepcopy、Bash 语法、父/候选完整构建通过：均为 22,771,111 参数/711 states；
  实际 497 optimizer groups 保持 `[0.1,1,2,20]` 倍率。
- 真实固定 GPU0/1 的 4-iter DDP smoke 四个 loss/grad 均有限；checkpoint 为
  364,512,758 bytes，SHA-256
  `e38b5e39287711d4e4ae9db27427596e21f2fde8a253016a6485e6922569b897`，
  iterative-cls/DN 已训练、642 个浮点 tensor 全有限、fatal=0。fresh formal 的 screen、
  torchrun、双 rank、GPU0/1 与工作目录一致；e1 iter50 LR/loss/grad 为
  `1.0e-7/21.5442/108.2206`，total、DN、encoder proposal 全有限且 fatal=0，五项动态
  门槛完成，正式登记 `RUNNING/TO_E72`。
...
- 178 `0810_06` One-Cycle peak×2.5 e20 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `49.939/41.658/62.407`，det 为 `57.763/51.299/67.177`，sum `107.702`；pair
  mAP/AP50 `0.291977/0.491287`，both-independent `0.335227/0.539099`。相对 e16，
  cls/det HOTA 上升 `0.933/2.761`，DetA 上升 `0.264/1.268`，AssA 上升
  `2.024/4.751`，四项 AP 继续上升，故继续峰后 e24。
- 178 e20 的 392,154,356-byte checkpoint SHA-256 为
  `87b0817404860f3473ea885a3ab4f11c71dc2b1629aef5c26c0fc7cd10d32a27`；642 个浮点
  tensor、iterative-cls/DN、5416/50、28/108/50 与 `async_done=1` 全闭环，TrackEval
  268.4 秒；verifier margin 为 `-5.324/-4.836/-10.160`，训练恢复 e21 iter300。
- 99 `0810_08` warmup12-cosine e16 为 cls `49.327/41.727/60.321`、det
  `56.697/50.140/66.435`，sum `106.024`；pair `0.276969/0.480504`，both-independent
  `0.320977/0.533255`。相对 e12 HOTA 双升 `2.575/3.492`；同 e16 相对 178 HOTA 高
  `0.321/1.695`，det AssA 高 `4.009`，AP 接近；它在 e16 已只低 178 e20
  `0.612/1.066`，支持约四个 epoch 的成熟前移，继续 e20/e24 检验保持性。
- 99 e16 的 386,529,654-byte checkpoint SHA-256 为
  `126c020a491ea8ed41e0ac958408acd9172cb469d1f6481bce86f93608fdbe88`；同样通过 642
  tensor、iterative-cls/DN、5416/50、28/108/50 与 async 全闭环，TrackEval 307.7 秒；
  verifier margin `-5.936/-5.902/-11.838`，训练恢复 e17 iter300。

## 2026-08-10 22:37 CST：252 One-Cycle peak×2.0 e16 完整闭环

- 252 `0810_07` e16 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `45.071/36.674/57.882`，det 为 `50.068/44.936/57.857`，sum `95.139`；pair
  mAP/AP50 `0.227497/0.402997`，both-independent `0.270990/0.460384`。相对 e12，
  cls/det HOTA 继续上升 `6.025/6.100`，DetA 上升 `3.824/4.452`，AssA 上升
  `8.920/8.858`，四项 AP 同步上升，故它不是停滞节点。
- 同 e16 相对 178 peak×2.5，252 cls/det HOTA 低 `3.935/4.934`，DetA 低
  `4.720/5.095`，AssA 低 `2.501/4.569`，pair/both mAP 低 `0.051343/0.053434`；
  当前全指标被更高峰值支配，但两条 One-Cycle 均在 epoch21.6 达峰，故仍保留 252 至
  e20 与峰后 e24，不以 e16 直接否决。
- 386,570,998-byte checkpoint SHA-256 为
  `c828c6b675b41210ae2601099ad98dd6d68fdf044fb1d05c4426ab5c93561884`；642 个浮点
  tensor、iterative-cls/DN、5416/50、28 CSV、108 非空文件、50 predictions 与
  `async_done=1` 全部通过。TrackEval 受 NFS `rpc/nfs_wait_bit_killable` 延迟后在
  991.9 秒自然完成，训练恢复到 e17 iter900；verifier margin 为
  `-10.192/-12.531/-22.723`，只说明该中期点未过最终线。
...
- 178 `0810_06` One-Cycle peak×2.5 e12 同一 checkpoint 的 cls
  HOTA/DetA/AssA 为 `44.024/37.766/54.705`，det 为
  `49.377/46.686/53.771`，sum `93.401`；pair mAP/AP50
  `0.242287/0.418168`，both-independent `0.290527/0.480515`。相对 e8，cls/det
  HOTA 上升 `11.530/12.004`，DetA 上升 `8.783/9.752`，AssA 上升
  `15.324/14.767`，显示仍在快速形成而非停滞。381,129,460-byte checkpoint SHA-256
  为 `3b6dc39ab11428bca3ba8716d96131c0ea110f1e55f2a19ee4455e3c2bde0fd1`；642 个
  浮点 tensor、iterative-cls/DN、5416/50、28 CSV、108 非空文件、50 predictions 与
  `async_done=1` 全部通过。主训练已到 e15 iter750。
- 99 `0810_08` 标准 12e warmup + cosine e8 cls HOTA/DetA/AssA 为
  `40.201/33.203/51.170`，det 为 `46.708/41.073/55.034`，sum `86.909`；pair
  `0.196932/0.351540`，both-independent `0.239690/0.412321`。它比 178 同 e8 HOTA
  高 `7.707/9.335`，且在 e8 已比 252 e12 高 `1.155/2.740`，是当前形成最快路线。
  375,537,142-byte checkpoint SHA-256 为
  `3534d07fb7eb86012cece4d4991a89fae0b6e3080ba322f39ad06fdd7125d04b`；有限性、
  iterative-cls/DN、5416/50、28/108/50 与 `async_done=1` 闭环，主训练到 e10 iter1000。
- 252 `0810_07` One-Cycle peak×2.0 e12 cls HOTA/DetA/AssA 为
  `39.046/32.850/48.962`，det 为 `43.968/40.484/48.999`，sum `83.014`；pair
  `0.187661/0.341973`，both-independent `0.237962/0.419204`。同点被 178 peak×2.5
  全面领先 HOTA `4.978/5.409`，DetA、AssA 与 AP 也均更低；但两条 One-Cycle 的峰值均在
  epoch 21.6，故 e12 仍不是停线点。381,086,774-byte checkpoint SHA-256 为
  `e65260a3890375b1f847ff4a03a466d609e21c34b139d0900b642306c1e3877d`，其余完整性门槛
  均通过。TrackEval 曾因 `rpc_wait_bit_killable` 共享存储等待延长至 916.5 秒，进程自行恢复
  并自然完成；主训练未受影响，已到 e13 iter800。
- 三节点 verifier 均为 `fallback_pass=false`，不能证明最终 e72 失败。后续顺序为 178/252
  e16 与 peak 后 e24、99 e12 及后续 cosine 成熟点；197 仍 SSH 超时，WSD 不越过动态门槛。
...
- 99 `0810_08` e4 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `27.170/20.136/39.522`，det 为 `36.995/28.502/48.859`，sum `64.165`；pair
  mAP/AP50 `0.090551/0.186640`，both-independent `0.126042/0.249651`。相对同为
  2×4 的 252 `0810_07` e4 `14.874/31.414`，HOTA 提前形成 `+12.296/+5.581`，AP
  也同步更高，支持 12e 标准 warmup-cosine 更快进入有效区间，但 e4 仍不是停线或胜负门槛。
- 99 的 369,988,342-byte `epoch_4.pth` SHA-256 为
  `815f8d84d727a43b1a5e8f0cc0657fb90ab77055ff8c5b645042b9a5712e5115`；642 个模型
  浮点 tensor 全有限，iterative-cls/DN 已训练。5416/50 检测、28 CSV、108 个非空文件、
  50 predictions 与 `async_done=1` 齐全，TrackEval 223.2 秒自然完成；主训练恢复 e5
  iter550，只用动态 GPU0/2，GPU1 当前空闲但未额外占用。
- 252 `0810_07` e8 cls HOTA/DetA/AssA 为 `31.712/24.229/44.724`，det 为
  `37.834/31.449/46.794`，sum `69.546`；pair mAP/AP50
  `0.115459/0.222711`，both-independent `0.157338/0.294269`。相对 178 peak×2.5
  同点，cls/det 为 `-0.782/+0.461`；两支 AssA 更高、DetA 和 AP 更低，显示较温和 peak
  此时更偏关联而定位形成更慢，不能只用 HOTA 单项解释。
- 252 的 375,593,718-byte checkpoint SHA-256 为
  `9e82d591dca3722d8adb860899f0db7cd54ac6390364d86393ffa7f08f4e420d`；642 个模型
  浮点 tensor、iterative-cls/DN 审计通过。5416/50、28/108/50、`async_done=1` 完整，
  TrackEval 331.2 秒自然结束；训练恢复 e9 iter300，固定仅 GPU0/1，GPU2/3 空闲。
- 同期 178 已到 e10 iter500；三线正式 loss、DN、Encoder proposal 与 grad 均有限，
  fatal=0。两条 One-Cycle 均继续 e12/e16 与 peak 后成熟节点，99 继续 e8/e12，均不以
  e4/e8 直接否决。

## 2026-08-10 19:10 CST：178 标准 One-Cycle epoch-8 完整闭环

- `0810_06` e8 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `32.494/28.983/39.381`，det 为 `37.373/36.934/39.004`，sum `69.867`；pair
  mAP/AP50 `0.156090/0.298093`，both-independent `0.207748/0.380997`。相对 e4
  `0.300/1.493` 已形成有效检测与轨迹，但相对直接 product-tangent 父线 e8
  `46.673/53.922` 仍低 `14.179/16.549`，四项 AP 也同步偏低，定位和关联均尚未成熟。
- 375,609,076-byte `epoch_8.pth` SHA-256 为
  `2fefd4127eeb83c144c4ba9009341e0141f317693c3db073b3346f17b99bfafb`；642 个
  浮点模型 tensor 全有限，iterative-cls 与 DN absolute heads 已训练。检测为
  5416 records/50 sequences；TrackEval 第二任务 235.9 秒自然完成，28 CSV、108 个非空
  文件、50 predictions 和 `async_done=1` 齐全。
- 评测后主进程已恢复到 e9 iter300，LR/loss/grad 为
  `8.7133e-5/11.0997/44.7710`，仍只占动态 GPU0，GPU1 空闲，运行时 fatal=0。该标准
  One-Cycle 的上升段持续到 epoch 21.6，故 e8 仅登记形成速度诊断，不直接否决，继续收集
  e12/e16 与后续成熟节点。
- 同期 252 `0810_07` 到 e8 iter700、99 `0810_08` 到 e4 iter750；两线正式日志、授权
  GPU 与数值均健康，尚未生成各自下一 checkpoint。
...
- `0810_08` queue screen `3349124` 在 18:06:11 发现动态 GPU0/2 候选，连续三次通过
  `<1024 MiB/<10%` 后于 18:06:51 启动准确的 2×4 DDP smoke。四步 loss
  `12.9372/19.5001/19.6385/21.1806`、grad
  `102.8727/97.0811/86.7174/91.0894`，total、DN 与 Encoder proposal 均有限。
- 364,512,886-byte `iter_4.pth` SHA-256 为
  `e3c0370a1b1e9393556ea9b1e7d558b3025dbb1c0bd015bb30f4255b9735f04a`；meta
  `0/4`，model/EMA `711/712` keys、642 个浮点 tensor 全有限，iterative-cls/DN 已训练，
  optimizer 497 groups、两个标准 scheduler、message hub 与 EMA 状态齐全。
- queue 在 smoke 完整结束后于 18:07:39 启动 fresh formal，并在 18:08:49 观察到正常训练
  区间后自然退出。独立复核确认 formal screen/main `3364119/3364122`、双 rank/workers 存活，
  只占动态 GPU0/2 各约 19.2 GiB；e1 iter50 LR/loss/grad 为
  `1.0e-7/21.5510/127.1880`，DN 与 Encoder proposal 全有限，运行时 fatal=0。正式配置中
  `LinearLR(begin=0,end=12,start_factor=0.000375)` 和
  `CosineAnnealingLR(begin=12,end=72,T_max=60,eta_min_ratio=1e-4)` 均由 checkpoint 管理。
  五项门槛全部通过，状态由 `QUEUED` 升为 `RUNNING/TO_E72`；GPU1 外部任务未触碰。
...
- commit `0adee95` 新增 `0810_08` 严格队列：任意两张卡必须连续三次满足显存
  `<1024 MiB` 且利用率 `<10%`，随后才运行目标配置的真实 2×4 DDP smoke；smoke launcher
  自身必须生成 `iter_4.pth` 并通过 iterative-cls/DN 与全 tensor 有限性检查，队列才会在
  同一对动态 GPU 上启动 fresh formal，并等待正式日志出现正常 `Epoch(train)` 区间。
- 99 远端 checkout 已无 LFS 变更、HEAD `0adee95`、clean，queue/两份 launcher 均通过
  远端 `bash -n`。queue screen `3349124` 在 17:26:25 存活；启动前后 GPU 为
  `0:12 MiB/0%`、`1:20345 MiB/97%`、`2:约23408 MiB/96–97%`，故只记录等待，未创建
  smoke/formal workdir，也未占用 GPU。
- 同期主线 178 已进入 e4i100，252 到 e3i450；两端 screen/训练/GPU 映射和 fatal 扫描
  正常，尚无 e4 checkpoint。继续等待 178 完成 e4 后的 checkpoint、检测与 TrackEval。

## 2026-08-10 17:19 CST：标准 warmup-cosine 后备完成静态闭环

- `0810_08` 只替换 LR 曲线：12 epoch 标准线性 warmup 从父线相同的 `1e-7`
  安全起点升至 base-group `2.6666667e-4`，随后 60 epoch 使用 MMEngine
  `CosineAnnealingLR(T_max=60, eta_min_ratio=1e-4)`。连续近似的名义积分为
  `0.5×12×(8/3) + 0.5×60×(8/3) = 96` 个父线 epoch；它比当前 One-Cycle 的
  21.6-epoch 上升段更早进入有效 LR 区间，但不使用人工倍率跳变。
- 隔离 commit `a78b239` 已部署到 99 的 clean detached checkout。首次普通 clone 被无关
  历史 LFS 动图 smudge 中断；精确核验并只清理新建失败副本后，以 skip-smudge 无损重建。
  本地/远端 launcher `bash -n`、配置 deepcopy、父子 model/train data/train cfg/hooks
  等同性均通过；optimizer 除 peak LR 这一曲线参数外完全相同。
- 完整模型构建仍为 22,771,111 参数/711 states。实际 optimizer 为 497 组，peak 四档
  `[2.6667e-5,2.6667e-4,5.3333e-4,5.3333e-3]`，初始化四档
  `[1e-8,1e-7,2e-7,2e-6]`，所有组共同缩放 `0.000375`，原 `0.1/1/2/20`
  倍率逐项保持。
- 99 在 17:04 的 GPU0 空闲，但 GPU1/2 各有约 20.3/23.4 GiB 外部任务，197 仍 SSH
  超时；因此不满足真实双卡 smoke 条件。`0810_08` 只登记
  `STATIC_VALIDATED/WAITING_2_FREE_GPUS`，完成真实 DDP smoke、有限 checkpoint 和
  formal iter50 五门槛前不得登记 RUNNING。
- 同时审计主线：178 `0810_06` 到 e3i800，LR/loss `1.9607e-5/16.1195`；252
  `0810_07` 到 e3i200，LR/loss `1.2837e-5/12.5755`。两线 screen/训练进程/GPU 映射
  合法、fatal=0，尚未生成 e4 checkpoint；继续等待完整诊断节点，不以早期 loss 或
  裁剪前 grad norm 停线。
...
## 2026-08-10 03:01 CST：全模型 EMA-lag 收束失败并转向 decoder-frozen 单因素

- fraction 0.50 完整结果为 cls HOTA/DetA/AssA `55.171/45.664/68.807`、det
  `62.225/54.607/73.386`，sum `117.396`；pair mAP/AP50
  `0.316672/0.533237`、both-independent `0.355568/0.570005`。5416/50、28/108/50
  完整，严格 margin `+0.734/-0.168/-0.434`，verifier rc=2 后 runner 自然完成并释放 GPU0。
  原点/0.25/0.50 三点二次拟合的 det 与 sum 峰值约为 `62.372/117.613`，均低保底线，
  因而不继续无信息的全模型 fraction 扫描。
- 0.25 相对原点同时提高 cls/det DetA `+0.517/+0.590`，但 det AssA 下降 `0.463`；据此
  `0810_03` 只增加一个块级约束：105 个 `decoder.*` 浮点 state 保持原 EMA，其余 537 个
  state 仍按 0.25 校正。模型图、参数、类别处理、loss 和推理计算均不变，非 class-aware、
  无 reweight。
- 新工具的 8 项单元测试通过；91,468,881-byte payload SHA-256
...
- `0808_02` e16 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `47.432/38.067/61.774`，det 为 `57.103/49.726/67.955`，绝对和
  `104.535`。pair mAP/AP50 为 `0.247341/0.430837`，both-independent 为
  `0.287731/0.477695`；386,531,943-byte `epoch_16.pth` 的 SHA-256 为
  `04cf9522c15ad3a9646077b7aca4192237c8274e4b2ee61d14bda2f71c8312e7`。
  5416 条检测、50 序列、28 CSV、108 个非空 TrackEval 文件和异步完成日志全部闭环；
  严格验收器按预期返回 2。
- 该点相对 e12 为 `+0.285/+2.766`，但相对直接 product-tangent 父线 e16
  `51.220/57.370` 仍为 `-3.788/-0.267`；其 cls 差距较 e12 继续扩大，联合差距仅从
  `-4.543` 收窄到 `-4.055`。因此在 e4/e8/e12/e16 四个完整节点后精确 TERM
  PGID `2811864`（成员 `23→0`），不是 e4/e8 早停；GPU0/1 连续两次为
  `1 MiB/0%`。
- 单因素后继 `0808_07` 保持最终 product-tangent 模型、参数/state、loss、数据和推理计算
  完全不变：e0–11 使用父线 `1e-4`，e12–23 使用
  `1.2037682266e-4`，e24–71 使用 `1.4490579434e-4`，使 72 epoch 的名义 LR
  积分严格等于父线 96 epoch，同时避免一次性 40% 跳变。它仍为 class-agnostic、无
  reweight、无新层/attention/loss。
- clean detached `adca76a` 的真实双卡 smoke 产生 loss
  `12.9372/19.4996/19.6153/21.2276`、grad
  `102.8418/91.7395/85.7160/93.2128`；364,513,383-byte `iter_4.pth`
  SHA-256 为 `941d0447924eed1ad544b3499523e52425297ced40b95add9d6caefb9b43ec3d`，
  iterative-cls/DN 已训练且 642 个浮点张量全有限。fresh formal screen/main
  `3583196/3583197` 已在动态 GPU0/1 达到 e1 iter50：LR/loss/grad 为
  `2.5488e-6/21.4029/112.8655`，total、DN、encoder proposal 均有限，GPU 各约
  19.2 GiB 且满载，无 traceback/OOM/NaN/NCCL/unused-parameter fatal，故正式登记
  `RUNNING/TO_E72`。

## 2026-08-08 18:39 CST：178 decoder/head-only 加速 e16 保持恢复趋势

- `0808_03` e16 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `49.515/41.094/62.034`，det 为 `56.831/50.640/65.907`，绝对和
  `106.346`。相对自身 e12 双 HOTA 为 `+1.517/+1.668`；相对直接 product-tangent
  父线 e16 `51.220/57.370` 只低 `1.705/0.539`，联合差距 `2.244`，明显优于
  `0808_02` e16 的联合差距 `4.055`。该点仍未达到 e72 目标，验收器按预期返回 2，
  但成熟趋势支持继续 e20/e24+，不停止。
- pair mAP/AP50 为 `0.273383/0.468078`，both-independent 为
  `0.315196/0.515004`；四项均较自身 e12 上升，但相对父线 e16
  `0.2914/0.5039` 与 `0.3379/0.5551` 仍有差距。5416 条检测、50 序列、28 CSV、
  108 个非空 TrackEval 文件和 50 个非空 prediction 全部闭环；异步 TrackEval 于
  18:37:05 自然结束，耗时 253.7 秒。
- 386,621,172-byte `epoch_16.pth` SHA-256 为
...
## 2026-08-08 19:54 CST：178 decoder/head-only 加速 e20 继续收窄父线差距

- `0808_03` e20 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `50.513/41.739/63.720`，det 为 `57.931/50.697/68.493`，绝对和 `108.444`。
  相对自身 e16 双 HOTA 为 `+0.998/+1.100`；相对直接 product-tangent 父线 e20
  `52.198/58.132` 只低 `1.685/0.201`，联合差距从 e16 的 `2.244` 继续收窄到
  `1.886`。检测 HOTA 已几乎追平父线且 cls/DetA/AssA 仍在增长，明确支持继续 e24+。
- pair mAP/AP50 为 `0.276800/0.471258`，both-independent 为
  `0.318240/0.516288`，相对自身 e16 分别约升 `0.0034/0.0032` 与
  `0.0030/0.0013`；虽然仍低父线 e20 `0.2958/0.5113` 与 `0.3395/0.5575`，但没有平台
  或反转证据。
- 392,144,884-byte `epoch_20.pth` SHA-256 为
  `a49d946cfa5e042c590c9ed6d2fd76d9f77cb6ffe11245df3baedf52b1bacbae`；meta
  `20/20760`，642 个浮点张量全有限，12 个 iterative-cls 参数组和 DN embedding 均为
  已训练非零状态。5416/50、28 CSV、108 个非空文件、50 predictions 完整；异步 TrackEval
  19:47:53→19:52:03 自然结束，耗时 250.2 秒，统一验收器按预期未过最终 e72 三门槛。
- 正式训练已恢复至 e21 iter300，只使用动态 GPU0。GPU1 新出现约 10.3 GiB 外部任务，未被
  本实验占用或触碰；178 的本目标资源使用仍严格为一张卡。

## 2026-08-08 20:36 CST：两条延迟-LR e8 与整体时钟压缩 e20 完整闭环

- 99 `0808_06` e8 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `42.129/34.567/54.207`，det 为 `46.937/42.547/53.729`，绝对和 `89.066`。
  pair mAP/AP50 为 `0.202483/0.373713`，both-independent 为
  `0.246844/0.436480`。相对直接 product-tangent 父线 e8 `46.673/53.922` 低
  `4.544/6.985`，且四项 AP 均低于父线；这是负的早期诊断，但首次 `1.4×` LR 切换尚在
  e12，故继续 e12/e16，不能以 e8 直接否决。
- 99 e8 的 375,549,430-byte checkpoint SHA-256 为
  `fc74e41074a472a99e2921e4ca830e0fe41db379f9333bdf38eda9efb1990755`，meta
  `8/8304`；model/EMA 各 642 个浮点张量全有限，iterative-cls 与 DN 均为已训练非零状态。
  5416/50、28 CSV、108 个非空文件、50 predictions 完整；TrackEval
  20:17:54→20:22:28 自然完成，耗时 273.6 秒。训练已恢复至 e9 iter1000，动态 GPU0/1，
  GPU2 为 10 MiB 空闲。
- 197 `0808_07` e8 为 cls `40.206/34.342/49.543`、det
  `46.565/43.035/52.302`，绝对和 `86.771`；pair mAP/AP50
  `0.198946/0.363087`，both-independent `0.244086/0.426887`。相对父线 e8 低
  `6.467/7.357`，也低 99 同点 `1.923/0.372`，说明 e4 的小优势没有延续；但 e12/e24
  两次 LR 提升均尚未发生，仍继续 e12/e16 作成熟判断。
- 197 e8 的 375,541,479-byte checkpoint SHA-256 为
  ...
  50 predictions 完整；TrackEval 20:26:45→20:32:14 自然完成，耗时 328.8 秒。
  训练已恢复至 e9 iter600，动态 GPU0/1，GPU2–5 均为空闲。
- 252 `0808_04` e20 为 cls `49.772/41.991/61.137`、det
  `57.365/50.966/66.730`，绝对和 `107.137`；相对自身 e16 双升
  `1.141/1.128`，但相对直接父线 e20 `52.198/58.132` 仍低 `2.426/0.767`，联合差距
  仅从 `3.722` 收窄至 `3.193`，并低 178 同点 `0.741/0.566`。pair mAP/AP50
  `0.274364/0.471042`、both-independent `0.318469/0.520843` 均较 e16 上升但仍低父线。
- 252 e20 的 392,030,454-byte checkpoint SHA-256 为
  `e5e920cd3730b4fd8e1df84d7c247ff396d0a9a6ca4b3922b22b54d7d925680f`，meta
  `20/20760`，model/EMA 各 642 个浮点张量全有限，5416/50、28 CSV、108 非空文件、
  50 predictions 完整；TrackEval 20:28:24→20:34:52 自然完成，耗时 387.1 秒。
  正式训练恢复至 e21 iter350，固定 GPU0/1，GPU2/3 仍为 `1 MiB/0%`。整体时钟压缩继续
  到 e24，但当前主优先级仍是 178 的 decoder/head-only 加速路线。
...
## 2026-08-08 16:21 CST：四线 e8 全量闭环；延迟升 LR 升为第一后备

- `0808_01/02/03/04` 的 e8 cls/det HOTA 分别为 `43.047/50.080`、
  `44.177/50.218`、`44.233/49.984`、`42.943/49.685`；同点和依次为
  `93.127/94.395/94.217/92.628`。相对直接 product-tangent 父线 e8
  `46.673/53.922`，四线 HOTA 均双低；其中 197 总和最高，178 仅低 `0.178`，252 从 e4
  四线最佳转为 e8 最弱。
- 四线 cls DetA/AssA 依次为 `35.180/55.317`、`37.007/54.797`、
  `37.236/54.933`、`36.233/53.447`；det 为 `44.431/58.864`、`45.748/57.122`、
  `45.638/56.774`、`45.089/56.616`。相对父线 cls `39.891/56.595`、det
  `47.262/64.136`，立即提高 LR 的共同瓶颈仍是 det AssA，且 DetA 也未保持 e4 的局部优势。
- pair mAP/AP50 依次为 `0.212543/0.384172`、`0.222107/0.405285`、
  ...
  `0.265917/0.445627`。父线对应 `0.252434/0.444294` 与 `0.303859/0.513938`，检测排序
  同样没有提前达到父线成熟度。
- 四个 e8 checkpoint 的 SHA-256 依次为 `6a92311b…2f5da`、`72680692…25af8e`、
  `bd00c286…75fc5`、`d949fca2…d69a80`；`last_checkpoint` 正确，iterative-cls residual
  与 DN absolute heads 有限且已训练。每条均有 5416/50 检测、28 CSV、108 非空文件、
  50 predictions，统一验收器按预期因 e8 绝对阈值返回 2。
- 该结果只是否定“e8 已完成加速”的判断，不是否定 decoder，也不允许在 e8 停止。四线已自然
  继续到 99 e10i100、197 e11i250、178 e10i200、252 e9i350，fatal=0，并完整保留 e12
  成熟窗口。若 e12 后释放一条筛选资源，优先给 `0808_06` 完成真实 smoke/checkpoint/formal
  iter50 五门槛；`0808_05` 因继承当前较弱的立即完整时钟压缩假设降为第二后备。
...
- 178 `0806_05 scale-orientation split product-tangent` e12 同一 checkpoint 的 cls
  HOTA/DetA/AssA 为 `44.366/34.045/60.856`，det 为 `49.670/42.362/60.165`，绝对和
  `94.036`。相对 e8 HOTA 继续上升 `+3.298/+4.669`，证明保留 e12 成熟窗口是必要的；
  但相对直接 product-tangent 父线 e12 `49.784/56.243` 仍低 `5.418/6.573`，且父线
  DetA/AssA `41.865/61.515`、`50.021/65.603` 四项均更强。pair mAP/AP50
  `0.2110/0.3670`、both-independent `0.2509/0.4149` 也不足，因此这是 e4/e8/e12
  三个完整节点后的结构性成熟否决，不是以 e4/e8 早停。
- 381,098,996-byte e12 checkpoint meta `12/12456`，model/EMA 711/712 states、各 642
  浮点张量全有限，optimizer 497 states、scheduler 与 iterative-cls/DN 语义完整；检测
  5416 records/50 sequences，TrackEval 28 CSV/108 非空文件和 `async_done=1` 齐全。
  PGID `175356` 精确 TERM，成员 `8→0`、screen 消失，GPU0 连续回到 1 MiB；GPU1 外部
  作业约 6.2 GiB，始终未触碰。
- 释放后按既定单因素顺序启动 `0806_02 log-SPD product-tangent`。隔离 checkout 为 clean
  detached `9c5018a`；正式目录启动前不存在；两 launcher `bash -n`、配置 deepcopy、
  22,771,111 参数/711 states 完整构建、既有真实 1×8 四步 smoke 的 364,507,508-byte
  checkpoint、642 浮点张量有限性与 iterative-cls/DN 语义均重新通过。
- fresh formal screen/main PGID `274432/274434` 仅使用动态 GPU0。09:34 iter50
  lr/loss/grad `2.5488e-6/21.0199/112.7436`，所有 DN 与 Encoder proposal 分量有限；
  iter100 为 `20.4108/90.2805`，正式日志 fatal 扫描为 0。进程、GPU0、正式日志与 iter50
  五门槛一致，登记 `RUNNING/E1/TO_E4+`；GPU1 外部作业保持约 6.2 GiB/92%，未触碰。
  e4/e8 只作诊断，不直接否决该 decoder。

## 2026-08-06 09:24 CST：252 e92 新高但严格未过；99 成熟交接至 0806_07

- 252 `0806_06` e92 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `55.534/46.365/68.575`，det 为 `62.430/54.766/73.626`，绝对和 `117.964`。
  cls、det 分别严格超过 Encoder `1.097/0.037`，但同点和距 `>118.330` 仍差 `0.366`，
  因此目标未完成。相对 e84 为 `+0.060/+0.008`，相对 e88 为 `+0.137/+0.027`，曲线从
  e88 回落后再创新高，故保持同模型、同优化状态继续既定 e96，而不是在 e92 停止。
- e92 pair mAP/AP50 `0.3226/0.5326`、both-independent `0.3609/0.5683`，较 e88 略回落。
  490,678,390-byte checkpoint meta `92/95496`，model/EMA 711/712 states、各 642 浮点张量
  全有限，optimizer 497 states、scheduler 与 iterative-cls/DN 语义完整。检测 5416/50、
  TrackEval 28 CSV/108 非空文件和 `async_done=1` 齐全；异步评测 376.6 秒自然完成。formal
  已恢复 e93，固定 GPU0/1，GPU2/3 均未使用。
- 99 `0804_17 quotient-anisotropy product-tangent` e24 cls HOTA/DetA/AssA
  `49.794/41.179/62.318`，det `57.460/50.101/68.276`，绝对和 `107.254`。相对 e20
  HOTA `+0.846/+0.534` 且 AP 继续升至 pair `0.2666/0.4706`、both-independent
  `0.3095/0.5206`，确认慢收敛窗口是必要的；但相对直接 product-tangent e24
  `52.478/58.771` 仍低 `2.684/1.311`，距严格三门槛 `4.643/4.933/9.076`。六个完整
  e4/e8/e12/e16/e20/e24 节点形成稳定结构性落后后，PGID `1995834` 精确 TERM，成员
  `23→0`、screen 消失，GPU0/1 连续归零，GPU2 始终未用。
- e24 的 397,519,734-byte checkpoint meta `24/24912`，model/EMA 711/712 states、各 642
  浮点张量全有限，optimizer 497 states、单 scheduler 与 iterative-cls/DN 语义完整；
  5416/50、28 CSV、108 非空文件和 `async_done=1` 齐全，TrackEval 自然完成。
- `0806_07 stratified product-tangent` 在 99 clean detached `ead7e1e` 上重新通过两配置
  deepcopy、两个 launcher `bash -n`、22,771,111 参数/711 states 父子完整构建。动态
  GPU0/1 四步真实 smoke loss `12.9372/19.4934/19.6385/21.2074`、grad
  `102.7695/107.0804/89.4597/90.1460`，DN/Encoder proposal 全有限；364,503,990-byte
  checkpoint meta `0/4` 的 642 浮点张量与 iterative-cls/DN 语义通过，fatal=0。
  formal screen/PGID `2037141/2037143` 的 iter50 lr/loss/grad
  `2.5488e-6/21.4216/101.1440`，GPU0/1 各约 19.2 GiB、GPU2 空闲，五门槛通过后登记
  `RUNNING/E1/TO_E4+`；e4/e8 只作诊断，不直接否决。
...
- 99 `0804_17 quotient-anisotropy product-tangent` e20 同一 checkpoint 的 cls
  HOTA/DetA/AssA 为 `48.948/40.416/61.552`，det 为 `56.926/49.543/67.702`，绝对和
  `105.874`。相对 e16，HOTA `+0.633/+1.342`，DetA `+0.474/+0.561`，AssA
  `+0.767/+2.470`，说明结构仍在收敛，不能在 e20 以慢收敛理由停止。
- pair mAP/AP50 从 e16 `0.2530/0.4489` 升至 `0.2606/0.4598`，both-independent 从
  `0.2972/0.5026` 升至 `0.3037/0.5110`。但相对直接 product-tangent 父线 e20
  `52.198/58.132` 仍低 `3.250/1.206`；相对严格 cls/det/和门槛分别差
  `5.489/5.467/12.456`，远未达目标。
- 392,024,246-byte `epoch_20.pth` meta `20/20760`，model/EMA 711/712 states、各 642
  个浮点张量全有限，optimizer 497 states、单 scheduler 与 loss scaler 完整；iterative-cls
  residual 和 DN absolute heads 有限且已训练。检测为 5416 records/50 sequences，TrackEval
  目录 28 CSV、108 个非空文件、50 个非空预测，`async_done=1`；异步评测于 07:56:11
  自然完成，耗时 275.4 秒。
- formal 已自然恢复并于 07:56 到 e21 iter300，loss/grad `9.8864/49.5497`，动态
  GPU0/1 保持约 21.4 GiB，GPU2 未用。由于 e12→e16→e20 的两项绝对 HOTA、DetA、AssA
  与 AP 尚持续上升，按 decoder 慢收敛约束继续到 e24，再做成熟判定和资源交接。
...
- 05:59 将提交 `445cac8` 相对 `d664a03` 的增量封装为
  `0806_06_445cac8_from_d664a03.bundle`，本地与 252 暂存副本 SHA-256 均为
  `1cddd6dd997633ff2ae23814c34b6bb90f091f600f2dddf92e3fa7fb55f437b7`。252 端只暂存 bundle；
  目标 checkout 与 formal workdir 再次核验均不存在，未执行 fetch/checkout/build/smoke，
  所以状态仍是 `PREPARED/NOT_DEPLOYED/NO_GPU`，且没有热更新 `0806_04` 存活仓库。
...
- e76 同一 checkpoint 的 cls HOTA/DetA/AssA 为 `55.233/45.956/68.400`，det 为
  `62.255/54.670/73.373`；cls 高于 Encoder `0.796`，det 仍低 `0.138`，绝对和
  `117.488` 距严格 `>118.330` 仍差 `0.842`。因此虽然较 e72 HOTA 再双升
  `+0.063/+0.090`，同点三门槛仍未完成。
- e72→e76 的 cls DetA/AssA 为 `+0.075/-0.069`，det 为 `+0.110/+0.060`；pair
  mAP/AP50 `0.3187/0.5331` 相对 e72 `+0.0001/-0.0004`，both-independent
  `0.3577/0.5699` 相对 e72 `-0.0004/-0.0010`。HOTA 改善很小但仍持续，检测 AP
  基本平台，尚不能在预定 e80 前停止成熟轨迹。
- 468,824,374-byte `epoch_76.pth` meta `76/78888`；model/EMA 分别 711/712 states，
  两者各 642 个浮点张量全部有限，optimizer 497 states 完整。验证为 5416 条/50 序列，
  `val_track_0001` 含 28 CSV、108 个非空文件和 50/50 个非空预测，`async_done=1`，
  TrackEval 耗时 398.6 秒。
- 固定 GPU0/1 的 PGID `1025568` 在异步评测期间已自然进入 e77 iter300，loss、grad、DN
  与 Encoder proposal 全有限，GPU2/3 保持 `1 MiB`。继续同一优化器/EMA/LR 轨迹到 e80，
  下一次只接受 e80 同 checkpoint 完整双评测作为终判。
- 99 动态 GPU0/1 的 `0804_17` 同时到 e4 iter400，正式项有限、GPU2 空闲；继续取得 e4
  checkpoint/检测/TrackEval，但 e4 只作结构诊断，仍须继续 e8/e12+。

## 2026-08-06 02:08 CST：178 的 0804_16 e12 成熟负结论并安全释放 GPU0

- e12 同一 checkpoint 的 cls HOTA/DetA/AssA 为 `46.594/36.616/61.916`，det 为
  `52.528/45.194/63.207`；相对强父线 e12 HOTA `48.289/54.539` 仍低
  `1.695/2.011`。绝对和仅 `99.122`，距严格 `>118.330` 为 `19.208`，且 cls/det
  分别低最终门槛 `7.843/9.865`，因此同 checkpoint 三项门槛均未通过。
- e8→e12 的 cls/det HOTA 回升 `+4.195/+4.929`，DetA 回升 `+2.534/+3.434`、AssA
  回升 `+6.427/+6.884`，说明 decoder 确在继续收敛；但 pair mAP/AP50
  `0.2299/0.4025`、both-independent `0.2718/0.4529` 相对强父线仍约低
  `0.0316/0.0623/0.0379/0.0700`。这是 e4/e8/e12 完整成熟窗口后的检测覆盖瓶颈，
  不是以 e4 或 e8 早停。
- 381,077,556-byte `epoch_12.pth` 的 meta 为 `12/12456`，iterative-cls/DN 已训练，
  642 个浮点 checkpoint 张量全有限；检测为 5416 条/50 序列，TrackEval 含 28 CSV、
  108 个非空文件、50/50 个非空预测、`async_done=1`，耗时 240.1 秒。
- 确认无残留异步 TrackEval 后，对唯一正式 PGID `4175891` 精确 TERM；成员归零、formal
  screen 消失，GPU0 回到 `1 MiB/0%`。GPU1 外部作业仍为约 5.7 GiB/高利用率，未触碰。
  99 的 `0804_17` 已独立运行，因此不把 178 资源提前交给 `0806_02`；后者仍严格排在
  `0804_17` 的 e12+ 成熟结论之后。
- 同步活体复核：252 固定 GPU0/1 的 `0806_01` 已到 e76 iter300，loss/grad、DN 与 Encoder
  proposal 全有限，GPU2/3 为 1 MiB；99 动态 GPU0/1 的 `0804_17` 到 e2 iter850，关键项
  全有限，GPU2 为 10 MiB。两路继续等待各自完整科学节点。
...
- 252 固定 GPU0/1 的 `0804_01 factorized product-tangent` e68 同一 checkpoint 的 cls
  HOTA/DetA/AssA 为 `54.853/45.619/68.213`，det 为 `61.883/54.379/72.931`。cls 严格超过
  Encoder `0.416`，det 仍低 `0.510`；同点和 `116.736` 距严格 `>118.330` 仍差 `1.594`，
  因此未达目标。
- 相对 e64 HOTA 双升 `+0.082/+0.247`；cls DetA/AssA 为 `-0.173/+0.658`，det 为
  `+0.065/+0.465`。pair mAP/AP50 `0.3158/0.5319`、both-independent
  `0.3550/0.5689`，相对 e64 四项也全升约 `+0.0015/+0.0026/+0.0008/+0.0011`，成熟曲线
  未平台，故固定 GPU0/1 继续最终 e72，23:09 已到 e69 iter500，GPU2/3 保持 `1 MiB/0%`。
- 457,893,430-byte checkpoint meta `68/70584`；12 个 iterative-cls residual 均非零，DN
  参数已训练，642 个浮点张量全部有限。检测与 TrackEval 为 5416/50、28 CSV、108 非空文件、
  50 preds、`async_done=1`，TrackEval 464.5 秒；历史只读诊断 PGID `854175` 经成员核对后精确
  TERM，正式训练 PGID `823929` 未受影响。
- 下一单因素 `0804_17 quotient-anisotropy product-tangent` 保留 0804_01 的中心切线，只把形状
  细节改为轴交换不变的双角各向异性平面运输，并逐帧严格保留 proposed log-area；它避免现有
  三维 `logw/logh/angle` 投影的单位混合，也不同于 0804_16 对整个形状增量做共识。
- 新结构零参数/state、class-agnostic、无 reweight，额外开销仅为终层常数逐元素运算。隔离
  checkout `/data1/users/litianhao01/PairMOT_quotientanisotropytangent_0804_17_static178` 为 clean
  detached `bd3f6fc`；定向测试 `1/1 OK`，formal/smoke config deepcopy、两 launcher
  `bash -n`、父/新整模构建全部通过：均 `22,771,111` 参数、711 state tensors、增量 0，目标
  workdir 均不存在。当前严格登记 `STATIC_VALIDATED/NOT_DEPLOYED/NO_GPU`；不得抢占 0804_16，
  后者仅用 GPU0 到 e4 iter400，GPU1 外部作业未动。

## 2026-08-05 22:41 CST：99 的 0804_15 e8 同 checkpoint 完整闭环

- 99 动态 GPU0/1 的 `0804_15 terminal quotient log-shape consensus` e8 同一 checkpoint 的
  cls HOTA/DetA/AssA 为 `40.836/33.809/52.106`，det 为 `46.935/42.481/53.812`。相对强父线
  `0803_13` e8，HOTA 为 `-4.166/-2.148`，cls DetA/AssA 为 `-5.328/-1.891`，det 为
  `-4.244/+0.458`；离散轴交换商空间只在 det AssA 留下轻微正差，主要损伤仍是检测覆盖/定位。
- e8 pair mAP/AP50 `0.1916/0.3533`、both-independent `0.2391/0.4247`，相对父线四项为
  `-0.041879/-0.071503/-0.046766/-0.069786`，与 DetA 负差方向一致。375,529,398-byte
  checkpoint 的 iterative-cls/DN 已训练且 642 个浮点张量有限；5416/50、28 CSV、108 非空
  文件、50 preds、`async_done=1` 完整，TrackEval 用时 320.5 秒。
- e8 只作中期机制归因，不直接否决 decoder；原 screen/PGID `1958427/1958429` 仍存活并已进入
  e9，GPU0/1 各约 21.4 GiB，继续收集 e12+，GPU2 未动。并行 252 固定 GPU0/1 到 e68
  iter300，178 `0804_16` 仅用 GPU0 到 e2 iter450；178 GPU1 外部作业与 252 GPU2/3 均未触碰。
...
- 99 动态 GPU0/1 的 `0804_13 hemisphere-fold center + log-shape consensus` e12 同一
  checkpoint 的 cls HOTA/DetA/AssA 为 `45.423/37.244/57.850`，det 为
  `52.430/46.839/60.743`。相对强父线 `0803_13` e12 的 HOTA 低 `2.866/2.109`；结合 e4
  `-1.953/+0.487`、e8 `-3.741/-1.818`，完整 e4/e8/e12 窗口已表明 hemisphere fold 没有
  形成成熟双侧增益，因此停止不是早期否决。
- e12 pair mAP/AP50 `0.2286/0.4179`、both-independent `0.2726/0.4724`，相对父线四项低
  `0.032939/0.046863/0.037110/0.050463`。381,030,262-byte checkpoint 的 iterative-cls/DN
  已训练，642 个浮点张量有限；5416/50、28 CSV、108 非空文件、50 preds、`async_done=1`
  完整，TrackEval 用时 277.3 秒。确认原 PGID `1891973` 的 23 个成员只属于该作业后精确 TERM，
  成员 `23→0`、screen 消失；GPU0/1 连续两次为 `10 MiB/0%`，GPU2 未触碰。
- `0804_15 terminal quotient log-shape consensus` 的 99 隔离 clean HEAD `491e329` 在释放后
  重新通过 config deepcopy、launcher `bash -n` 与 fresh workdir 审计。动态 GPU0/1 真数据
  4-iter DDP smoke 的 loss 为 `12.9355/19.4381/19.5973/21.2151`，grad 为
  `106.4100/93.1257/113.2730/105.4861`；total、DN、Encoder 均有限，364,505,910-byte
  checkpoint 的 iterative-cls/DN 已训练且 642 张量有限，致命扫描为 0。
- fresh formal screen/PGID `1958427/1958429` 于 20:03 启动；iter50 `0.9888 s/iter`、
  loss/grad `21.4132/114.8830`，7 个正式进程、GPU0/1 各约 19.2 GiB，total、DN、Encoder
  proposal 全有限且 fatal=0。五门槛全部通过后才登记 `RUNNING/TO_E4+`；20:05 已到 iter100，
  GPU2 保持空闲。该线继续收 e4/e8/e12+，不得以早期节点直接否决。
- 并行审计：252 固定 GPU0/1 的 e60 checkpoint 已落盘并通过训练语义与 642 张量有限性检查，
  同点检测/TrackEval 仍在运行；178 动态 GPU0 的 `0804_14` 到 e6 iter1000，GPU1 外部任务不动。

## 2026-08-05 19:40 CST：0804_14 e4 同点闭环，关联增益未覆盖 DetA 损失

- 178 动态 GPU0 的 `0804_14 hemisphere-boundary center + log-shape consensus` e4 同一
  checkpoint 的 cls HOTA/DetA/AssA 为 `33.145/26.651/44.064`，det 为
  `37.230/33.238/43.278`。相对强父线 `0803_13` e4，HOTA 为 `+0.296/-0.089`；cls
  DetA/AssA 为 `-0.031/+0.143`，det 为 `-1.710/+2.035`。最近半球边界避免了 fold 的分类
  大幅退化，但检测侧仍以覆盖/定位换关联，尚未形成双侧结构增益。
- pair mAP/AP50 为 `0.1430/0.2685`，both-independent 为 `0.1871/0.3400`；相对强父线分别
  `+0.0031/+0.0072/-0.0010/+0.0009`，与 TrackEval 的“AP 基本持平、det AssA 增加”一致。
  369,974,964-byte checkpoint 的 iterative-cls/DN 已训练，642 个浮点张量全部有限；
  5416 records/50 sequences、28 CSV、108 非空 TrackEval 文件、50 preds 和
  `async_done=1` 完整，异步 TrackEval 用时 219.6 秒。
- e4 只登记机制归因，不作为 decoder 否决点。原 PGID `4074056`、GPU0 训练已安全续到 e5
  iter350，继续收集 e8/e12+；GPU1 外部任务保持不动。并行实际进度为 99 动态 GPU0/1 e12
  iter450、252 固定 GPU0/1 e60 iter200，GPU2 与 GPU2/3 均未触碰；`0804_15` 继续保持
  `STATIC_VALIDATED/NOT_DEPLOYED/NO_GPU`。

## 2026-08-05 18:58 CST：0804_15 quotient-space 形状共识双端口静态闭环

- e56 的 252 product-tangent 仍被不改中心的 terminal log-shape 父线同点支配，而 99/178 的
  多种中心耦合持续表现为 AssA 局部增加、DetA 与 AP 下降。因此后继不再增强中心约束；
  `0804_15 terminal quotient log-shape consensus` 只修复旋转框
  `(w,h,theta) ~ (h,w,theta+pi/2)` 的参数等价类：用 detached reference 的最短主轴 lift 判断
  是否交换当前帧 width/height 的 log-size tangent，随后做对称均值并映回各帧自身表示；周期角
  仍沿用成熟父线，中心、分类、DN、loss、attention、层数和递归 reference 均不变。
- 该操作零参数/state、frame-swap 等变、class-agnostic、无 reweight/gate/scale 扫描；无轴交换时
  严格退化为原 terminal log-size mean，额外开销仅是常数个 wrap、比较、flip 与 elementwise
  运算，不会明显增加计算量。
- 99/178 的隔离 checkout 分别为
  `/data/users/wangying01/lth/PairMOT_quotientlogshape_0804_15_99` 与
  `/data1/users/litianhao01/PairMOT_quotientlogshape_0804_15_178`，均为 clean detached HEAD
  `491e329`。两端定向测试各 `2/2 OK`，formal/smoke config deepcopy、四份 launcher `bash -n`
  通过；父/新整模均为 `22,771,111` 参数、711 state tensors，增量 0。
- 两端的 smoke/formal 四个目标 workdir 全部不存在，状态严格为
  `STATIC_VALIDATED/NOT_DEPLOYED/NO_GPU`。它仅作为 99 e12 与 178 `0804_14` e12 成熟后继，不得
  抢占当前训练；届时仍须重新确认动态空闲卡，并依次通过真实 smoke、checkpoint 与 formal
  iter50 五门槛后才能登记 RUNNING。
- 18:58 实际进度为 252 固定 GPU0/1 e58 iter100、99 动态 GPU0/1 e10 iter100、178 动态 GPU0
  e3 iter450；252 GPU2/3、99 GPU2 未用，178 GPU1 外部任务未触碰。
...
## 2026-08-08 21:11 CST：178 decoder/head-only e24 继续收窄父线差距

- `0808_03` e24 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `51.215/42.178/64.593`，det 为 `58.367/50.939/69.202`，绝对和 `109.582`。相对自身
  e20 双升 `0.702/0.436`；相对直接 product-tangent 父线 e24 `52.478/58.771` 只低
  `1.263/0.404`，联合差距由 e20 的 `1.886` 继续收窄到 `1.667`。这是当前最强成熟策略，
  保留到 e28+，不能在仍持续恢复时释放。
- pair mAP/AP50 为 `0.279480/0.475666`，both-independent 为
  `0.320369/0.520240`，四项均高于自身 e20 的 `0.276800/0.471258` 与
  `0.318240/0.516288`；DetA、AssA 与 AP 同向改善，未见用关联换检测或反向退化。
- 397,671,732-byte `epoch_24.pth` 的 SHA-256 为
  `525b3dd347dfc7da31e7175c76ed86d93069d9bfa16f0702c8fbf5f3b04f8725`；meta
  `24/24912`，model/EMA 分别 711/712 keys、各 642 个浮点张量全有限，iterative-cls 与
  DN 状态已训练。检测为 5416 records/50 sequences；TrackEval 产生 28 CSV、108 个非空
  文件和 50 个非空 predictions，耗时 255.9 秒自然结束，统一验收器按未达 e72 的预期返回 2。
...
## 2026-08-08 22:29 CST：decoder/head-only e28 首次同点双超直接父线

- 178 `0808_03` e28 同一 checkpoint 的 cls HOTA/DetA/AssA 为
  `52.850/43.936/65.433`，det 为 `59.702/51.790/71.166`，绝对和 `112.552`。相对自身
  e24 双升 `1.635/1.335`；cls DetA/AssA 分别升 `1.758/0.840`，det 分别升
  `0.851/1.964`，说明检测与关联共同增长，而不是单侧交换。
- 相对直接 product-tangent 父线 e28 `52.641/58.986`，首次实现同点双超
  `0.209/0.716`，绝对和领先 `0.925`。分量上候选 cls/det DetA 低父线
  `0.532/0.723`，但 AssA 高 `1.286/2.560`，证明 decoder/head-only 学习率加速的收益主要
  来自更快形成稳定轨迹关联。其 e28 总和也仅低父线 e32 `112.629` 的 `0.077`，已近似压缩
  4 个父线 epoch。
- pair mAP/AP50 `0.2992/0.5105`、both-independent `0.3406/0.5543`，均较自身 e24
  明显提升；虽然仍略低父线 e28 的 `0.3001/0.5156` 与 `0.3442/0.5622`，HOTA 已由 AssA
  反超，且没有 AP 退化趋势。距 e72 保底门槛仍差 cls `1.587`、det `2.691`、绝对和
  `5.278`，因此必须继续 e32/e36+，不能登记达标。
- 403,198,580-byte `epoch_28.pth` SHA-256 为
  `614243d4a87efc01983fc7dcf9e22742f081aa451cff095ec4ad94a60d04447c`；meta
  `28/29064`，model/EMA 分别 711/712 keys、各 642 个浮点张量全有限，iterative-cls/DN
  已训练。5416/50、28 CSV、108 非空文件、50 predictions 完整，TrackEval 252.9 秒自然
  结束。训练已恢复 e29 iter350，只用动态 GPU0；GPU1 未用。

## 2026-08-08 22:59 CST：两种延迟全局 LR 的 e16 首次切换响应闭环

- 99 `0808_06` e16 cls HOTA/DetA/AssA 为 `48.335/40.549/60.188`，det 为
  `55.763/49.857/64.564`，绝对和 `104.098`；相对 e12 双升 `2.933/4.159`。pair
  mAP/AP50 `0.2560/0.4553`、both-independent `0.2994/0.5084`，四项也较 e12 上升，
  说明 e13 后一次 `1.4×` LR 跳变产生真实恢复，而非只抬高轨迹指标。
- 197 `0808_07` e16 为 cls `48.551/40.821/59.843`、det
  `55.024/49.892/62.710`，绝对和 `103.575`；相对 e12 双升 `1.361/1.914`。pair
  `0.2574/0.4558`、both-independent `0.3023/0.5088`。相对 99 同点，cls 高 `0.216`，
  det 低 `0.739`、总和低 `0.523`；两边 AP 几乎相同，但 99 det AssA 高 `1.854`，说明
  单次大跳变在首四轮更快形成检测轨迹关联，没有出现预想的 AssA 崩落。
- 相对直接父线 e16 `51.220/57.370`，99 仍低 `2.885/1.607`，197 低
  `2.669/2.346`；相对 178 decoder/head-only e16 `49.515/56.831`，99 低
  `1.180/1.068`、197 低 `0.964/1.807`。两条都尚未追平局部加速，不升级为主候选；但只
  消费了四轮新 LR，继续 e20，且 197 还需到 e24 才发生第二阶段切换。
- 99/197 checkpoint 分别为 386,561,078/386,529,319 bytes，SHA-256 分别为
  `8b6e9deec38bc3da98de3219fce9c1c6bf5d7f8ad486d288b433b19640490c64` 与
...
## 2026-08-08 23:56 CST：197 分阶段延迟 LR e20 完整闭环

- `0808_07` e20 cls HOTA/DetA/AssA 为 `50.650/42.356/62.730`，det 为
  `56.444/50.569/65.114`，绝对和 `107.094`。相对 e16 双升 `2.099/1.420`，cls DetA/AssA
  分别升 `1.535/2.887`，det 分别升 `0.677/2.404`，说明第一阶段 `1.2038×` LR 后检测与
  关联都在恢复，不是 HOTA 内部交换。
- 相对直接 product-tangent 父线 e20 `52.198/58.132` 仍低 `1.548/1.688`；相对 178
  decoder/head-only 同点 `50.513/57.931`，cls 高 `0.137`、det 低 `1.487`，总和低
  `1.350`。pair mAP/AP50 `0.2692/0.4706`、both-independent `0.3142/0.5221`，较 e16
  `0.2574/0.4558` 与 `0.3023/0.5088` 同步提升，但仍不足以升级为主候选。
- 392,028,519-byte checkpoint SHA-256
...
## 2026-08-09 01:12 CST：252 局部 Adam e4 与 197 分阶段 LR e24 闭环

- 252 `0808_08` e4 cls HOTA/DetA/AssA 为 `31.106/24.911/41.502`，det 为
  `37.307/31.243/45.775`，总和 `68.413`；pair mAP/AP50 `0.13298/0.24654`、
  both-independent `0.17370/0.31179`。相对直接父线 e4 HOTA 低 `4.168/6.542`，相对
  旧 decoder/head LR×4/3 e4 `34.176/38.801` 也低 `3.070/1.494`，说明压缩 Adam 记忆在
  最早期进一步扰动形成过程；但它尚未到 e8/e12 成熟窗口，严格继续，不以 e4 否决。
- 369,970,550-byte e4 checkpoint SHA-256 为
  `4a6691c7c783f99c7cf423272ab7f141e5a1288f713df6dddcb522ac421e2b04`；642 个浮点张量
  全有限、iterative-cls/DN 已训练，5416/50、28/108/50 与 `async_done=1` 完整。训练只用
  固定 GPU0/1 继续，GPU2/3 仍空闲。
- 197 `0808_07` e24 cls HOTA/DetA/AssA 为 `52.091/43.546/64.231`，det 为
  `58.225/51.554/68.041`，总和 `110.316`；较 e20 双升 `1.441/1.781`，相对直接父线 e24
  只低 `0.387/0.546`。pair `0.27739/0.48424`、both-independent
  `0.32262/0.53496`；e24 也是第二次 LR 提升前的基准，e25 已实际使用 `1.4491e-4`，故明确
  保留到 e28 评价完整分阶段策略。
- 197 e24 checkpoint 397,524,007 bytes，SHA-256
...
## 2026-08-09 02:30 CST：197 e28 与 178 局部延迟线 e4 全量闭环

- 197 `0808_07` e28 cls HOTA/DetA/AssA 为 `52.580/44.265/64.141`，det 为
  `58.980/52.026/69.173`，总和 `111.560`。相对自身 e24 双升 `0.489/0.755`；相对直接
  product-tangent 父线 e28 仅低 `0.061/0.006`，但尚未形成把成熟度提前到 e72 所需的正增益。
  相对旧 decoder/head-only 局部高 LR e28 `52.850/59.702` 仍低 `0.270/0.722`，差异来自
  AssA 低 `1.292/1.993`，尽管 DetA 高 `0.329/0.236`。pair mAP/AP50
  `0.285742/0.495156`、both-independent `0.330566/0.545248`，也均低父线和旧局部线。
- 403,016,103-byte e28 checkpoint SHA-256 为
  `aac5233a6d1496003daec669d2e91c0372a802d08c971a868d0c063eeb3ada3e`；meta
  `28/29064`，model/EMA `711/712` keys、各 642 个浮点张量全有限，iterative-cls/DN 已训练。
  5416/50、28 CSV、108 非空文件、50 predictions 与 `async_done=1` 完整；TrackEval 338.5 秒
  自然结束。训练已恢复到 e30 iter200、动态 GPU0/1、fatal=0。第二阶段仅消费四轮，因此继续
  e32 作为最低完整成熟窗口，不在 e28 单点释放 197。
- 178 `0809_01` e4 cls HOTA/DetA/AssA 为 `32.955/25.403/44.408`，det 为
  `41.857/31.323/57.964`，总和 `74.812`；pair mAP/AP50 `0.143762/0.268205`、
  both-independent `0.184286/0.331246`。该点位于 e12 局部 LR 切换之前，只用于建立单卡物理
  轨迹的早期基准，不能据此否决 decoder；继续 e8、e12 及切换后的 e16。
- 369,986,740-byte e4 checkpoint SHA-256 为
  `4a2e195c8de24a9943c1d14f6a1c5e651952856ce09ac6e0f6bf004980684ef5`；meta
  `4/4152`，model/EMA `711/712` keys、各 642 个浮点张量全有限，iterative-cls/DN probe
  为已训练非零状态。5416/50、28/108/50 完整，TrackEval 211.1 秒自然结束；训练已恢复 e5
  iter350，只用 GPU0，GPU1 空闲、fatal=0。

## 2026-08-09 02:34 CST：252 局部 Adam 时钟 e8 完整闭环

- `0808_08` e8 cls HOTA/DetA/AssA 为 `40.166/33.358/50.568`，det 为
  `47.240/42.304/54.498`，总和 `87.406`；较自身 e4 双升 `9.060/9.933`，说明早期扰动后
  仍在快速恢复。相对直接 product-tangent 父线 e8 `46.673/53.922` 低 `6.507/6.682`，
  相对旧 decoder/head-only LR e8 `41.972/48.178` 低 `1.806/0.938`。pair mAP/AP50
  `0.197302/0.346075`、both-independent `0.240257/0.404885`，也低父线
  `0.252434/0.444294` 与 `0.303859/0.513938`，因此记为一致的负早期诊断，但不在 e8 否决。
- 375,537,462-byte checkpoint SHA-256 为
  `2c9d29b07be111012bc3648e9708cdb7308a70818fe2d3aeb73978be403e54ed`；meta
  `8/8304`，model/EMA `711/712` keys、各 642 个浮点张量全有限，17 个 iterative-cls/DN
  probe 均为已训练非零状态。5416/50、28 CSV、108 非空文件、50 predictions 与
  `async_done=1` 完整；TrackEval 361.0 秒自然结束。
...
## 2026-08-09 03:42 CST：178 局部延迟 LR e8 全量诊断闭环

- `0809_01` e8 cls HOTA/DetA/AssA 为 `42.853/34.088/57.072`，det 为
  `49.918/42.530/61.415`，总和 `92.771`；较自身 e4 双升 `9.898/8.061`。相对 252 的局部
  Adam 时钟 e8 `40.166/47.240` 双高 `2.687/2.678`，pair mAP/AP50
  `0.210202/0.366499` 与 both-independent `0.252391/0.422217` 也分别高
  `0.012900/0.020424` 和 `0.012134/0.017332`，说明不压缩 Adam 记忆的早期形成更稳定。
- 相对直接 product-tangent 父线 e8 `46.673/53.922` 仍低 `3.820/4.004`，四项 AP 也低；
  但该点完全位于 e12 局部 LR 切换前，只建立单卡物理轨迹基准。它尚未验证核心干预，明确继续
  e12 边界与 e16 首四轮局部 `1.4×` 响应，不以 e8 否决 decoder。
- 375,582,388-byte checkpoint SHA-256 为
  `5e3296522e332a37d8d1d70fc99938beeed33c51a90eb80004b93a081c6eefed`；meta
  `8/8304`，642 个浮点张量全有限且 iterative-cls/DN 已训练。5416/50、28 CSV、108 非空
  文件、50 predictions 与 `async_done=1` 完整；TrackEval 240.5 秒自然结束。formal 已恢复
  e9 iter350，只用 GPU0，GPU1 空闲、fatal=0。

## 2026-08-09 03:59 CST：252 局部 Adam 时钟 e12 成熟负证据

- `0808_08` e12 cls HOTA/DetA/AssA 为 `45.569/37.291/57.726`，det 为
  `52.337/46.717/60.617`，总和 `97.906`；较自身 e8 双升 `5.403/5.097`，尚未平台。
  但相对直接 product-tangent 父线 e12 `49.784/56.243` 低 `4.215/3.906`，相对旧
  decoder/head-only 局部 LR e12 `47.998/55.163` 也低 `2.429/2.826`；双 DetA、双 AssA
  均未形成补偿，说明局部 Adam 记忆压缩目前延缓而非加速成熟。
- pair mAP/AP50 `0.231117/0.401379`、both-independent `0.272619/0.453119`；相对直接
  父线约 `0.2758/0.4786` 与 `0.3214/0.5310` 全低，也低旧局部 LR e12
  `0.264565/0.457265` 与 `0.308771/0.508690`。该结论不是跟踪阈值或 HOTA 内部交换。
- 381,036,022-byte checkpoint SHA-256 为
  `ec1bb0a7b5b35e14f62de6c4e8978a29fd561e4aa6c99578f5c32ba4035a2e2d`；meta
  `12/12456`，642 个浮点张量全有限且 iterative-cls/DN 已训练。5416/50、28 CSV、
  108 非空文件、50 predictions 与 `async_done=1` 完整；TrackEval 在最慢 252 上耗时
  453.1 秒自然结束。
- e4/e8/e12 三点均为负，但 e8→e12 仍双升且 decoder 允许慢收敛；按预先约定保留固定
  GPU0/1 到 e16 作最后成熟复核，而不是在单个 e12 停线。formal 已恢复 e13 iter450，
  GPU2/3 仍空闲、fatal=0；该候选优先级降至四线最低，不再在 252 上新增并行路线。
