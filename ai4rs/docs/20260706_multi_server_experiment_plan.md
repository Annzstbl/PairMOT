...
Last updated: 2026-08-13 21:06 CST.
...
At 17:12 CST on August 13, 178 closes e68 at cls/det HOTA
`55.557/61.850` (sum `117.407`), DetA/AssA `45.932/69.940` and
`54.507/72.548`, and pair mAP/AP50 `0.3174/0.5344`. Versus e64 it changes
`+0.054/-0.147`; cls clears target by `0.294`, while det/sum remain short by
`0.749/0.455`. The 458,360,244-byte checkpoint, 51 detection files and
28 CSV/108 TrackEval files are complete, with finite e69 training continuing
on GPU0. Keep the line to the required e72 despite e60 remaining its local
best sum.

At 17:02 CST on August 13, 99 closes e68 at `54.946/62.240` (sum
`117.186`), down `0.336/0.063` from e64; DetA/AssA is `45.607/68.196`
and `54.464/73.523`, with pair mAP/AP50 `0.3120/0.5299`. The floor
reduces late det regression but does not stabilize cls, so retain through the
required e72. 197 closes e52 at `54.327/61.639` (sum `115.966`), changing
`-0.128/+0.272` from e48; DetA/AssA is `45.524/66.526` and
`53.738/73.072`, with pair mAP/AP50 `0.3121/0.5249`. Its 25% floor only
diverges after e60. Both checkpoints, 51 detection files and 28 CSV/108
TrackEval files are complete; finite training continues in e69/e53.
...
`53.718/72.463`, and pair mAP/AP50 `0.3111/0.5230`. Versus e44, cls
falls `0.292` while det rises `0.108`. The 430,238,183-byte checkpoint,
5,416/50 detection, 51 detection files, 28 CSV/108 TrackEval files and
356.9-second evaluation are complete; finite training continues in e49 on
GPU4/5. Since the 25% floor differs only after e60, retain it through e72
rather than interpreting this stable-phase fluctuation as a terminal result.

At 15:57 CST on August 13, both leading lines close e64. The 99 floor-cosine
line reaches cls/det HOTA `55.282/62.303` (sum `117.585`), DetA/AssA
`45.881/68.496` and `54.435/73.729`, and pair mAP/AP50 `0.3115/0.5289`;
it rises `0.220/0.192` from e60, clears cls by `0.019`, and is short on
det/sum by `0.296/0.277`. The 178 WSD line scores `55.503/61.997`
(sum `117.500`), DetA/AssA `45.932/69.572` and `54.480/72.926`, and pair
mAP/AP50 `0.3183/0.5364`; it falls `0.304/0.293` from its local-best e60.
Both checkpoints, detection exports, 28 CSV/108 TrackEval files and roughly
301-second evaluations are complete, with finite training continuing in e65.
Retain both to e68/e72; neither the e64 dip nor the e60 local peak replaces
the required same e72 checkpoint.

At 15:17 CST on August 13, fixed-252 closes e44 at cls/det HOTA
`55.079/61.692` (sum `116.771`), DetA/AssA `46.021/67.586` and
`53.958/72.967`, and pair mAP/AP50 `0.3146/0.5285`. Versus e40, both
HOTA values rise by `0.626/0.622`. The checkpoint, 5,416/50 detection,
28 CSV/108 TrackEval files are complete. Shared `/data4` reads and NFS writes
made asynchronous closure take 1294.5 seconds, but it completed normally;
training continues in e45 on fixed GPU0/1. Retain this line through its
post-e48 decay window while prioritizing 178 e64.
...
`45.369/66.364` and `52.850/71.969`, and pair mAP/AP50 `0.3046/0.5173`;
this is a `1.519/1.074` HOTA rise from e36. Fixed-252 reaches
`54.453/61.070` (sum `115.523`), DetA/AssA `46.218/65.614` and
`53.534/72.106`, and pair mAP/AP50 `0.3105/0.5258`, a `1.869/0.768`
rise from e36. Both checkpoints, detection exports, 28 CSV/108 TrackEval
files and 345.2/451.7-second asynchronous evaluations are complete. Training
continues in e41 on 197 GPU4/5 and fixed-252 GPU0/1. Do not touch the external
task observed on 252 GPU2. Retain both until their actual e48+/e60+ tails.

At 13:22 CST on August 13, both leading mature lines close e56. On 99, the
floor-cosine line scores cls/det HOTA `55.068/62.034` (sum `117.102`), with
DetA/AssA `46.007/67.709` and `54.306/73.291`, and pair mAP/AP50
`0.3115/0.5276`; it slips `0.110/0.046` from e52 and remains short of the
strict target by `0.195/0.565/0.760`. On 178, smooth WSD scores
`55.485/61.952` (sum `117.437`), with DetA/AssA `45.823/69.695` and
`54.293/73.078`, and pair mAP/AP50 `0.3191/0.5337`; it improves
`0.233/0.169` from e52, clearing cls but still missing det/sum by
`0.647/0.425`. Both checkpoints, 5,416/50 detection, 51 detection files,
28 CSV/108 TrackEval files, and 282.9/253.8-second asynchronous evaluations
are complete. Training continues finitely in e57. Retain both through
e60/e64/e68/e72 because the objective is explicitly the same e72 checkpoint.

At 12:22 CST on August 13, 197 floor-25 WSD closes e36 at cls/det HOTA
`52.723/59.599` (sum `112.322`), DetA/AssA `43.544/66.080` and
`51.691/71.081`, with pair mAP/AP50 `0.2889/0.4900`. Relative to e32,
both HOTA values rise by `0.800/0.467`, sum by `1.267`. The 413,852,839-byte
checkpoint, 5,416/50 detection, 51 detection files, 28 CSV/108 TrackEval
files and 348.7-second asynchronous closure are complete; finite training
resumes at e37i400 on GPU4/5. Since the 25% floor differs only after e60,
retain the healthy trajectory through e72 rather than judging it at e36.
...
`53.387/71.074`, with pair mAP/AP50 `0.3055/0.5220`. Relative to e32, both
HOTA values rise by `0.419/0.679`. The 414,213,556-byte checkpoint, 5,416/50
detection and 250.5-second TrackEval are complete; training resumes at e37i300
on GPU0. At e36, it leads 99 in cls and AP but trails by `0.594` det and
`0.516` sum; keep both complementary lines through their actual tails to e72.
...
`50.157/68.584`, and pair mAP/AP50 `0.2665/0.4628`. Relative to e16, both
HOTA values and AP improve. The checkpoint, 5,416/50 detection and 383.3-second
TrackEval are complete; training resumes at e21i350 on GPU0/1. Retain it until
the e48+ cosine window. 99 has produced e36 and is validating, while 178 is
finishing e36 training; close those same-checkpoint evaluations next.
...
`113.169`; pair mAP/AP50 is `0.3040/0.5202`. Relative to e28, cls/det HOTA
rise by `0.583/0.793`. The 408,697,844-byte checkpoint, 5,416/50 detection,
and 258.1-second TrackEval are complete; formal training resumes at e33i300
on GPU0 only. Compared with 99 at e32, 178 trails HOTA sum by `0.686` but has
higher cls AssA, det DetA, and AP, while 99 has `1.649` higher det AssA. Keep
both complementary schedules through e36 and their actual decay windows.

At 05:29 CST on August 13, 99 floor-cosine closes e32 at cls
HOTA/DetA/AssA `53.339/44.762/65.416` and det `60.516/53.094/71.387`, sum
`113.855`; pair mAP/AP50 is `0.3016/0.5127`. Relative to e28, both HOTA values
rise by `0.700/0.476`, and DetA, AssA, and AP improve together. The
408,511,606-byte checkpoint, 5,416/50 detection, and 278.4-second TrackEval
are complete; formal training resumes at e33i250. This remains the strongest
current line and has not yet reached its low-LR floor tail, so preserve it to
e36 and mature decay. 178 has produced its e32 checkpoint and is running the
single-GPU 1,354-iteration validation before same-point comparison.

At 05:21 CST on August 13, 197 floor-25 WSD closes e12 at cls
HOTA/DetA/AssA `46.128/37.797/59.120` and det `53.493/47.492/62.448`, sum
`99.621`; pair mAP/AP50 is `0.2384/0.4156`. Relative to e8, both HOTA values
rise by `3.901/6.066`, and the complete checkpoint, 5,416/50 detection, and
339.7-second TrackEval confirm healthy convergence. Because the 25% floor only
diverges from the zero-floor WSD during the post-e60 cosine tail, this early
node does not decide the strategy; formal training continues at e13. The 99
line has produced its e32 checkpoint and completed 677/677 validation inference,
with asynchronous TrackEval next; 178 and 252 continue e32/e17 respectively.
...
At 21:02 CST, the 197 WSD line closes epoch 56 at cls HOTA/DetA/AssA
`53.993/44.659/66.949` and det `61.814/53.922/73.136`, sum `115.807`.
Pair mAP/AP50 is `0.3110/0.5241`, both-independent is `0.3469/0.5564`;
5,416/50 detection, 28 CSV, 108 nonempty evaluation files, 50 predictions,
and the async completion metric are present. The sum is only `0.002` below
e52: cls DetA retreats while both association values remain stable or improve.
Do not stop at this plateau because WSD decay begins after e60; continue e60,
...
At 16:21 CST, 197 e40 is fully closed. The same `epoch_40.pth` gives cls
HOTA/DetA/AssA `53.109/43.911/65.935` and det `60.938/52.981/72.528`,
sum `114.047`; pair mAP/AP50 remains `0.2992/0.5055`, and
both-independent is `0.3407/0.5482`. Detection is 5,416/50, TrackEval is
28 CSV, 108 nonempty files and 50 predictions with `async_done=1.0`.
The 419,229,735-byte checkpoint has meta epoch/iter `40/41520`, model/EMA
711/712 states, SHA-256
`24e20316824769497a9bcb592f794f5db784028e723d846a5e2495e579ea2835`,
and all 642 floating checkpoint tensors are finite. This early WSD point is
...
with pair mAP/AP50 `0.3153/0.5230` and both-independent `0.3532/0.5576`.
All 5,416 records, 50 detection files, 50 trajectory files, 28 CSV files,
108 nonempty evaluation files, and `track/async_done=1.0` are complete;
TrackEval finishes naturally in 166.7 seconds. The 430,449,716-byte
checkpoint has SHA-256
`f3b097eea376e6e2fcf796710273c9c991cad1905caae8f898427a57dc1c507f`,
meta `epoch=48/iter=49824`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against e44, cls/det HOTA improve by `0.421/0.128` and
the sum gains `0.549`. Against warmup12-cosine e48, both improve by
`0.017/0.302`, retaining a `0.319` sum advantage. The remaining goal gaps
are `0.430/0.321/0.751`; training resumes naturally at e49 iteration 250
and continues to e52 and later mature checkpoints. Data-disk headroom is
about 7.0 GiB and will be checked at each node.

At 11:47 CST, e44 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `54.412/44.631/68.943` and det `62.150/54.387/73.501`,
with pair mAP/AP50 `0.3138/0.5219` and both-independent `0.3521/0.5584`.
All 5,416 records, 50 detection files, 50 trajectory files, 28 CSV files,
108 nonempty evaluation files, and `track/async_done=1.0` are complete;
TrackEval finishes naturally in 162.8 seconds. The 424,964,660-byte
checkpoint has SHA-256
`87ce0e6a0606cccf6d7be23518d00aa17ebcc79095e7fe4da1e4e277da2ffc0d`,
meta `epoch=44/iter=45672`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against e40, cls/det HOTA change by `-0.094/+0.154` and
the sum still gains `0.060`, while all four AP summaries dip slightly.
Against warmup12-cosine e44, they change by `-0.180/+0.322`, retaining a
`0.142` sum advantage. Training resumes naturally at e45 iteration 300;
continue to e48 to distinguish a mature fluctuation from a trend.

At 10:50 CST, e40 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `54.506/44.894/68.723` and det `61.996/54.467/73.065`,
with pair mAP/AP50 `0.3153/0.5269` and both-independent `0.3543/0.5643`.
All 5,416 records, 50 detection files, 50 trajectory files, 28 CSV files,
108 nonempty evaluation files, and `track/async_done=1.0` are complete;
TrackEval finishes naturally in 166.3 seconds. The 419,474,932-byte
checkpoint has SHA-256
`ebe35f2c4634049e64f08f864daa2d896104aba9765aa6ac8fbe4c770f22ccf3`,
meta `epoch=40/iter=41520`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against e36, cls/det HOTA improve by `1.851/0.360` and
the sum gains `2.211`, with all four AP summaries increasing. Against
warmup12-cosine e40, they change by `-0.035/+0.285`, retaining a `0.250`
sum advantage. The remaining goal gaps are `0.757/0.603/1.360`; training
resumes naturally at e41 and continues to e44 and later mature checkpoints.

At 09:55 CST, e36 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `52.655/43.231/67.442` and det `61.636/53.823/73.165`,
with pair mAP/AP50 `0.302992/0.507713` and both-independent
`0.341717/0.546061`. All 5,416 records, 50 detection files, 50 trajectory
files, 28 CSV files, 108 nonempty evaluation files, and
`track/async_done=1.0` are complete; TrackEval finishes naturally in 165.8
seconds. The 413,988,020-byte checkpoint has SHA-256
`b1966b8fbece648df39bf5b22add76ce1f0ee4aa8107d4fedd8c6f4f80ad9325`,
meta `epoch=36/iter=37368`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against e32, cls/det HOTA improve by `0.439/0.437`, the
sum gains `0.876`, and all DetA, AssA, and AP summaries recover. Against
warmup12-cosine at e36, they change by `-0.253/+0.754`, retaining a `0.501`
sum advantage. Training is healthy at e37 iteration 550 with about 9.0 GiB
data-disk headroom. Continue to e40 and later mature checkpoints.
...
with pair mAP/AP50 `0.304872/0.510172` and both-independent
`0.344541/0.550450`. All 5,416 records, 50 detection files, 50 trajectory
files, 28 CSV files, 108 nonempty evaluation files, and
`track/async_done=1.0` are complete; TrackEval finishes naturally in 168.1
seconds. The 402,998,260-byte checkpoint has SHA-256
`2fcf004ca507577a6f577227613b21aef6b1072f65c9d4bbb0a51bef5b8ba2af`,
meta `epoch=28/iter=29064`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against warmup12-cosine at e28, cls/det HOTA improve by
`0.411/0.889`, with a `1.300` sum advantage; against the original
product-tangent parent they change by `-0.256/+1.621`, retaining a positive
`1.365` sum advantage. Training is healthy at e29 iteration 900 with about
11 GiB data-disk headroom and no exact fatal signature. Continue to e32 and
later mature checkpoints.

At 07:00 CST, e24 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `52.115/43.151/65.666` and det `60.061/52.801/70.770`,
with pair mAP/AP50 `0.302287/0.506918` and both-independent
`0.342950/0.549071`. All 5,416 records, 50 detection files, 50 trajectory
files, 28 CSV files, 108 nonempty evaluation files, and
`track/async_done=1.0` are complete; TrackEval finishes naturally in 169.1
seconds. The 397,515,124-byte checkpoint has SHA-256
`e71d3f7b896b4184d1b5201513c499a174ecac1012a04663cdfd9ce01725b2f7`,
meta `epoch=24/iter=24912`, 711/712 model/EMA states, and 2,776 finite
floating tensors. Against warmup12-cosine at e24, cls/det HOTA improve by
`0.265/1.062`; against the original product-tangent parent they change by
`-0.363/+1.290`, retaining a positive `0.927` sum advantage. Training resumes
to e25 iteration 450 with about 12 GiB data-disk headroom. Continue to e28 and
later mature checkpoints rather than stopping at e24.

At 06:02 CST, e20 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `51.772/42.780/65.435` and det `59.525/52.529/69.866`,
with pair mAP/AP50 `0.299258/0.503352` and both-independent
`0.340627/0.546798`. All 5,416 records, 50 detection files, 50 trajectory
files, and `track/async_done=1.0` are complete; TrackEval finishes naturally
in 166.9 seconds. The 392,033,012-byte checkpoint has SHA-256
`e2c0d417c058ae7c0e7558448d823d6552e96a17f8a3261eca48c6b02942636f`,
meta `epoch=20/iter=20760`, 711/712 model/EMA states, 497 optimizer states,
and 2,776 finite floating tensors. Against warmup12-cosine at e20, cls/det
HOTA improve by `0.544/1.489`; against the original product-tangent parent,
they change by `-0.426/+1.393`, retaining a positive `0.967` sum advantage.
Training resumes to e21 iteration 350 with about 12 GiB data-disk headroom.
Continue to e24 and later mature checkpoints rather than rejecting on one cls
rollback.

At 05:08 CST, e16 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `51.488/42.891/64.243` and det `58.779/52.124/68.640`,
with pair mAP/AP50 `0.296979/0.499316` and both-independent
`0.340233/0.546737`. All 5,416 records, 50 detection files, 50 trajectory
files, and `track/async_done=1.0` are complete; TrackEval finishes naturally
in 167.2 seconds. The 386,538,932-byte checkpoint has SHA-256
`9a4528a18610c9b784496ebc0aa188dbc82f568edcb04ba7192d1b26eec55571`,
meta `epoch=16/iter=16608`, 711/712 model/EMA states, 497 optimizer
states/groups, and no nonfinite value among any model, EMA, or optimizer
floating tensors. Against standard warmup12-cosine at e16, cls/det HOTA
improve by `2.161/2.082`, with positive DetA, AssA, and AP comparisons. It
also exceeds the original product-tangent parent e16 by `0.268/1.409`.
Training resumes to e17 iteration 550 with about 12.3 GiB data-disk headroom.
Continue to e20 and mature checkpoints; e16 is not a final acceptance point.

At 04:07 CST, e12 is fully closed. The same checkpoint gives cls
HOTA/DetA/AssA `50.576/41.670/63.969` and det `57.205/50.649/66.839`,
with pair mAP/AP50 `0.284429/0.474770` and both-independent
`0.327946/0.523463`. All 5,416 records, 50 detection files, 50 trajectory
files, and `track/async_done=1.0` are complete; TrackEval finishes naturally
in 163.6 seconds. The 381,046,644-byte checkpoint has SHA-256
`df41476c72b23c42b3a9bb68add417b88e942ebabd6d6552e1010a6eda9459ff`,
meta `epoch=12/iter=12456`, 711/712 model/EMA states, 497 optimizer
states/groups, and no nonfinite value among 1,284 floating model/EMA tensors.
Against standard warmup12-cosine at e12, cls/det HOTA improve by
`3.824/4.000`, while all DetA, AssA, and AP comparisons remain positive.
It also exceeds the original product-tangent parent e12 HOTA by
`0.792/0.962`. Training resumes to e13 iteration 250. Continue to e16 and
mature checkpoints; e12 is not sufficient evidence for final e72 success.
...
`42.562/32.002/58.675`, with pair mAP/AP50 `0.157039/0.287951` and
both-independent `0.192337/0.340811`. Detection contains 5,416 records over
50 sequences; all 50 detection and trajectory files are nonempty, TrackEval
finishes naturally in 135 seconds, and `track/async_done=1.0`. The
369,985,140-byte checkpoint has SHA-256
`e545879480146a2f94529b1914d8b9bc4919462ff5d9b6cbe7a1f960a5330339`,
meta `epoch=4/iter=4152`, 711/712 model/EMA states, 497 optimizer
states/groups, and no nonfinite value among 1,284 floating model/EMA tensors.
Eleven validation-only match diagnostic fields are NaN because they are not
applicable in this validation path; AP, tracking metrics, checkpoint tensors,
and all training losses are finite, with no traceback, OOM, NCCL, or runtime
error. Training resumes to e5 iteration 350 on GPU0. This is an early
diagnostic point only and does not reject the decoder; continue to e8 and
mature checkpoints.
...
e12 `39.046/43.968`; dynamic-99 standard warmup-cosine reaches e8
`40.201/46.708`. Every checkpoint, finite-state, iterative-cls/DN, 5,416/50
detection, 28 CSV/108 non-empty/50 prediction and async TrackEval gate is
closed. 99 forms fastest and 178 dominates 252 at matched e12, but One-Cycle
still rises to epoch 21.6, so both continue to e16 and post-peak e24 while 99
continues to e12. The 252 e12 TrackEval recovered naturally from an NFS RPC
wait and finished in 916.5 seconds; training was unaffected. Server 197 still
times out and WSD remains unlaunched.
...
At 16:37 CST on 2026-08-10, the active objective remains the paper-ready
scheduler replacement: keep the final product-tangent model, optimizer, EMA,
data, loss, global batch and inference graph unchanged, and require one
epoch-72 checkpoint to reach cls HOTA `>=55.263`, det HOTA `>=62.599`, and sum
`>=117.862`. A mandatory optimizer-group audit invalidated the first scalar
OneCycleLR attempts `0810_04/0810_05`: scalar `eta_max` collapsed all 497
optimizer groups to one LR and erased the inherited paramwise multipliers.
Both jobs were stopped precisely in epoch 1 and are retained only as
`INVALID_SCALAR_ETA_MAX` protocol records.

Fresh successors `0810_06` (dynamic 178 GPU0, 1x8, peak factor `2.5`) and
`0810_07` (fixed 252 GPU0/1, 2x4, peak factor `2.0`) run commit `552668d`.
The adapter only expands each factor over the scheduler-construction-time group
LRs and delegates the phase boundaries, cosine annealing and checkpoint state
to MMEngine's standard OneCycleLR. All 497 group ratios are asserted before GPU
launch. Model, optim-wrapper configuration, EMA/hooks, data and train loop are
equal to the parent; both full builds remain 22,771,111 parameters/711 states.
Real-data smoke checkpoints are finite and formal iter50 LR/loss/grad is
`1.0003e-5/20.0431/82.6103` on 178 and
`8.0023e-6/20.6372/94.4325` on 252. Strict fatal scans are empty and process,
...
`-4.271/+0.012` versus Encoder. Its cls/det DetA/AssA are
`26.971/40.785` and `34.129/45.148`; pair mAP/AP50 is `0.1437/0.2727`.
The checkpoint, 5416 detection records, 50 sequences, 28 CSV files, 108
non-empty evaluation files, and async completion marker are complete. This
is an early scale-factor attribution point, not a rejection: dynamically
selected 197 GPU2/3 continues PGID `2540932` through e8/e12 while GPU5
external work remains untouched.
...
`+0.416/+1.397` versus terminal mean geometry. Complete checkpoint, AP, 50-sequence
TrackEval, 28 CSV files, 108 evaluation files, and async completion were verified. The early
classification slowdown and small detection gain remain insufficient for rejection; dynamic 99
GPU0/1 continues PGID `1673454` to e8/e12 while GPU2 external work remains untouched.
...
verified before exact PGID `1016336` was stopped after the full e4/e8/e12 mature window.
The zero-parameter `0804_03` terminal log-size-only successor isolates multiplicative scale
consensus from the angle-only run on 99. It passed its targeted unittest, config deep copies,
launcher syntax, zero-state-delta full build, real two-GPU smoke, finite checkpoint audit, and
fresh formal iter50. It is RUNNING on dynamically selected 197 GPU2/3 under PGID `2540932`;
GPU5 external work remains untouched. On the slow lane, fixed-GPU 252 `0803_30` completed e8
at `40.934/47.531` with all artifacts closed and continues to e12 under PGID `798989`; e8 is
recorded as a negative intermediate signal, not a rejection point.

At 23:05 CST, 99 `0803_29` completed its e4/e8/e12 mature window and remained
dual-negative at e12 (`45.384/51.334` versus original decoder `47.395/54.436`). Complete
checkpoint, AP, 50-sequence TrackEval, 28 CSV files, 108 evaluation files, and async completion
were verified before exact PGID `1582836` was stopped. The orthogonal zero-parameter
`0804_02` terminal periodic-angle-only successor passed its targeted unittest, config deep
copies, source-provenance check, launcher syntax, zero-state-delta full build, real DDP smoke,
finite checkpoint audit, and fresh formal iter50. It is RUNNING on dynamically selected 99
GPU0/1 under PGID `1673454`.
...
at the aligned epoch and `4.105` short of the strict absolute sum target, the latest
dual rise contradicts a mature-plateau stop. Complete checkpoint, AP, 50-sequence
TrackEval, 28 CSV files, and 108 evaluation files were verified. The dynamically selected
178 GPU0 therefore remains on PGID `3151184` to e48; prepared `0803_30` consumes no GPU.
...
At 08:06 CST, `0803_21` completed epoch 4 at `30.158/37.094`, below the original decoder
by `4.148/1.496` with complete artifacts. It continues to epoch 8 and 12 under the slow
decoder-convergence rule, while semantic/geometric combination work stays deferred until a
mature positive semantic result exists.

At 08:29 CST, `0803_23` completed epoch 8 at `46.283/53.755`, exceeding the original
decoder by `+4.311/+5.577` and Encoder by `+1.014/+3.562` at the aligned checkpoint.
All AP and TrackEval artifacts are complete. It is now the first-priority trajectory and
continues unchanged to epoch 12; the shape-only and center-only variants remain PREPARED.

At 08:36 CST, mature `0803_13` completed epoch 32 at `53.642/60.531`, exceeding the
...
## 2026-08-08 17:55 CST：准备分段延迟 LR 的正交调度检验

- `0808_06` 检验“早期父线 + e12 单次加速”，下一单因素 `0808_07` 检验同一总 LR 积分下
  跳变幅度是否是关联损伤来源。它在 e12/e24 各乘
  `g=(sqrt(113)-1)/8=1.2037682266`，使72轮实际积分严格满足
  `12+12g+48g²=96`，首次 LR 变化比 `0808_06` 温和，最终倍率仅 `1.4491×`。
- 结构、数据与训练的其余部分完全不变，零推理增量。隔离提交 `adca76a` 已通过 config
  deepcopy、完整父/子构建、state 等同性、launcher 语法与实际 LR 序列审计；当前不占 GPU，
  不创建 smoke/formal 目录。
...
## 2026-08-09 03:59 CST：局部 Adam 时钟只保留到 e16 复核

- 252 e12 的 HOTA、DetA/AssA 与 AP 均被直接父线和旧局部 LR 成熟支配，说明压缩 Adam
  一阶/二阶记忆没有带来预期的早熟。由于 e8→e12 仍双升，按 decoder 慢收敛约束继续到 e16，
  但将其状态降为 `TO_E16_REVIEW`，不再默认占用到 e72。
...
- 252/e52已到`55.587/62.340=117.927`，只剩det缺口`0.259`，优先收集其e56/e60/e64/e68/e72；
...
- 后续收尾节点197/e64为`55.480/62.190=117.670`；252/e60为`55.415/62.405=117.820`，后者
  det/sum仅差`0.194/0.042`。继续且仅继续e68/e72。
- 197/e68已完整闭环为`55.238/62.217=117.455`，DetA/AssA为`45.258/69.292`与
  `54.006/74.041`，pair mAP/AP50为`0.3186/0.5350`；51检测文件与28 CSV/108 TrackEval
  文件完整。该点严格三项未达，按用户决定不扩展搜索，只继续197/e72及252既定收尾节点。
- 197/e72最终`55.379/62.180=117.559`，完整产物、2915张量有限性、进程退出与GPU4/5释放
  均闭环，严格失败。252/e64为`55.211/62.599=117.810`，det恰好达标而cls/sum均差`0.052`；
  最终判定只剩252/e68/e72，不新增实验。
- 252/e72最终`54.962/62.566=117.528`，DetA/AssA为`45.299/68.620`与`54.703/73.941`，
  pair mAP/AP50为`0.3168/0.5250`；2915张量有限、51检测文件、28 CSV/108 TrackEval文件、
  进程退出与固定GPU0/1释放全部闭环。所有保留线均已收尾，成熟标准调度目标未达；按用户决定
  不再启动新实验。
