...
| 197 | `0803_18 terminal geometry + semantic margins`（GPU4/5） | RUNNING；0803_11 e12 成熟停止后，smoke/iter50 五门槛通过，PGID `387859` | `0803_22 geometry + transported margins` PREPARED_LOCAL；`0803_20 full tangent + shared margins` PREPARED | `/data4/litianhao/PairMmot/workdir_197` |
...
- 0803_22 保持 0803_18 的终层 log-size/周期角，只把完全平均的 centered margins 替换为沿前序
  ...
  reweight 或额外层。
- 197 formal/smoke 配置、零状态构建审计与安全启动器已完成本地语法检查。状态
  `PREPARED_LOCAL/NO_GPU`；尚未同步、未建 workdir，不抢占当前 0803_18。
