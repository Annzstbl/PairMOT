...
def _build_decoder(num_layers: int = 2,
                   ...
                   bool = False,
                    ...
                    bool = False,
                    pair_shared_terminal_transport_covariant_frenet_product_tangent_refinement_decoder:
                    bool = False,
...
class TestPairRotatedRTDETRDecoder(unittest.TestCase):
    ...
    def test_scale_orientation_product_tangent_splits_metric_and_preserves_angle(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = projected._pair_transport_center_tangent_residual
        shape_projection = (
            projected._pair_transport_scale_orientation_shape_tangent_residual)
        with mock.patch.object(
                projected, '_pair_transport_center_tangent_residual',
                side_effect=center_projection) as center_mock, mock.patch.object(
                    projected,
                    '_pair_transport_scale_orientation_shape_tangent_residual',
                    side_effect=shape_projection) as shape_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
                1, decoder=projected,
                reg_branches_prev=reg_prev, reg_branches_curr=reg_curr)
        ...
        reference_prev = (
            torch.rand(2, 6, 5, device=self.device) * 0.4 + 0.3)
        reference_curr = (
            torch.rand(2, 6, 5, device=self.device) * 0.4 + 0.3)
        residual_prev = (
            torch.randn(2, 6, 5, device=self.device) * 0.08).requires_grad_()
        residual_curr = (
            torch.randn(2, 6, 5, device=self.device) * 0.08).requires_grad_()
        output_prev, output_curr = shape_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 2)
        ...
        decoded_prev = decode(reference_prev, output_prev)
        decoded_curr = decode(reference_curr, output_curr)
        ref_log_prev = torch.log(reference_prev[:, 2:, 2:4])
        ref_log_curr = torch.log(reference_curr[:, 2:, 2:4])
        tangent_prev = (
            torch.log(proposed_prev[:, 2:, 2:4]) - ref_log_prev)
        tangent_curr = (
            torch.log(proposed_curr[:, 2:, 2:4]) - ref_log_curr)
        common = 0.5 * (tangent_prev + tangent_curr)
        detail = 0.5 * (tangent_curr - tangent_prev)
        transport = ref_log_curr - ref_log_prev
        projected_detail = transport * (
            (detail * transport).sum(dim=-1, keepdim=True)
            / transport.square().sum(
                dim=-1, keepdim=True).clamp_min(1e-6))
        actual_prev = torch.log(decoded_prev[:, 2:, 2:4]) - ref_log_prev
        actual_curr = torch.log(decoded_curr[:, 2:, 2:4]) - ref_log_curr
        self.assertTrue(torch.allclose(
            actual_prev, common - projected_detail,
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
            actual_curr, common + projected_detail,
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
            decoded_prev[:, 2:, 4:], proposed_prev[:, 2:, 4:],
            atol=2e-6, rtol=2e-6))
        self.assertTrue(torch.allclose(
            decoded_curr[:, 2:, 4:], proposed_curr[:, 2:, 4:],
            atol=2e-6, rtol=2e-6))
        self.assertTrue(torch.equal(output_prev[:, :2], residual_prev[:, :2]))
        self.assertTrue(torch.equal(output_curr[:, :2], residual_curr[:, :2]))
        self.assertTrue(torch.equal(
            output_prev[:, 2:, :2], residual_prev[:, 2:, :2]))
        self.assertTrue(torch.equal(
            output_curr[:, 2:, :2], residual_curr[:, 2:, :2]))

        swapped_curr, swapped_prev = shape_projection(
            residual_curr, residual_prev,
            reference_curr, reference_prev, 2)
        self.assertTrue(torch.allclose(
            output_prev, swapped_prev, atol=1e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            output_curr, swapped_curr, atol=1e-6, rtol=1e-5))
        (output_prev.sum() + output_curr.sum()).backward()
        self.assertTrue(torch.isfinite(residual_prev.grad).all())
        self.assertTrue(torch.isfinite(residual_curr.grad).all())
    ...
    def test_quotient_anisotropy_product_tangent_preserves_area_and_transport(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = projected._pair_transport_center_tangent_residual
        quotient_projection = (
            projected._pair_transport_quotient_anisotropy_tangent_residual)
        with mock.patch.object(
                projected, '_pair_transport_center_tangent_residual',
                side_effect=center_projection) as center_mock, mock.patch.object(
                    projected,
                    '_pair_transport_quotient_anisotropy_tangent_residual',
                    side_effect=quotient_projection) as quotient_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
                1, decoder=projected,
                reg_branches_prev=reg_prev, reg_branches_curr=reg_curr)
        self.assertEqual(center_mock.call_count, 1)
        self.assertEqual(quotient_mock.call_count, 1)
        for reference in refs_prev + refs_curr:
            self.assertTrue(torch.isfinite(reference).all())
        ...
        residual_curr = (
            torch.randn(2, 6, 5, device=self.device) * 0.08).requires_grad_()
        output_prev, output_curr = quotient_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 2)
        ...
        decoded_prev = decode(reference_prev, output_prev)
        decoded_curr = decode(reference_curr, output_curr)
        ref_q_prev = anisotropy(reference_prev[:, 2:])
        ref_q_curr = anisotropy(reference_curr[:, 2:])
        tangent_prev = anisotropy(proposed_prev[:, 2:]) - ref_q_prev
        tangent_curr = anisotropy(proposed_curr[:, 2:]) - ref_q_curr
        common = 0.5 * (tangent_prev + tangent_curr)
        detail = 0.5 * (tangent_curr - tangent_prev)
        transport = ref_q_curr - ref_q_prev
        projected_detail = transport * (
            (detail * transport).sum(dim=-1, keepdim=True)
            / transport.square().sum(
                dim=-1, keepdim=True).clamp_min(1e-6))
        actual_prev = anisotropy(decoded_prev[:, 2:]) - ref_q_prev
        actual_curr = anisotropy(decoded_curr[:, 2:]) - ref_q_curr
        self.assertTrue(torch.allclose(
            actual_prev, common - projected_detail,
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
            ...
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
            torch.log(decoded_prev[:, 2:, 2:4]).sum(dim=-1),
            torch.log(proposed_prev[:, 2:, 2:4]).sum(dim=-1),
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
            torch.log(decoded_curr[:, 2:, 2:4]).sum(dim=-1),
            torch.log(proposed_curr[:, 2:, 2:4]).sum(dim=-1),
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.equal(output_prev[:, :2], residual_prev[:, :2]))
        self.assertTrue(torch.equal(output_curr[:, :2], residual_curr[:, :2]))
        self.assertTrue(torch.equal(
        ...
        swapped_curr, swapped_prev = quotient_projection(
            residual_curr, residual_prev,
            reference_curr, reference_prev, 2)
        self.assertTrue(torch.allclose(
            output_prev, swapped_prev, atol=1e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            output_curr, swapped_curr, atol=1e-6, rtol=1e-5))
        (output_prev.sum() + output_curr.sum()).backward()
        self.assertTrue(torch.isfinite(output_prev).all())
        self.assertTrue(torch.isfinite(output_curr).all())
        self.assertTrue(torch.isfinite(residual_prev.grad).all())
        self.assertTrue(torch.isfinite(residual_curr.grad).all())
    ...
    def test_log_spd_product_tangent_is_metric_coherent_and_parameter_free(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = projected._pair_transport_center_tangent_residual
        shape_projection = (
            projected._pair_transport_log_spd_shape_tangent_residual)
        with mock.patch.object(
                projected, '_pair_transport_center_tangent_residual',
                side_effect=center_projection) as center_mock, mock.patch.object(
                    projected, '_pair_transport_log_spd_shape_tangent_residual',
        ...
        decoded_prev = decode(reference_prev, output_prev)
        decoded_curr = decode(reference_curr, output_curr)
        ref_shape_prev = log_spd(reference_prev[:, 2:])
        ref_shape_curr = log_spd(reference_curr[:, 2:])
        tangent_prev = log_spd(proposed_prev[:, 2:]) - ref_shape_prev
        tangent_curr = log_spd(proposed_curr[:, 2:]) - ref_shape_curr
        common = 0.5 * (tangent_prev + tangent_curr)
        detail = 0.5 * (tangent_curr - tangent_prev)
        transport = ref_shape_curr - ref_shape_prev
        projected_detail = transport * (
            (detail * transport).sum(dim=-1, keepdim=True)
            / transport.square().sum(
                dim=-1, keepdim=True).clamp_min(1e-6))
        actual_prev = log_spd(decoded_prev[:, 2:]) - ref_shape_prev
        actual_curr = log_spd(decoded_curr[:, 2:]) - ref_shape_curr
        self.assertTrue(torch.allclose(
            actual_prev, common - projected_detail,
            atol=2e-5, rtol=2e-5))
        self.assertTrue(torch.allclose(
    ...
    def test_body_frame_product_tangent_is_terminal_and_parameter_free(self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
            device=self.device,
        ...
        center_projection = (
            projected._pair_transport_body_frame_center_tangent_residual)
        shape_projection = projected._pair_transport_shape_tangent_residual
        with mock.patch.object(
                projected,
                '_pair_transport_body_frame_center_tangent_residual',
                side_effect=center_projection) as center_mock, mock.patch.object(
                    projected, '_pair_transport_shape_tangent_residual',
                    side_effect=shape_projection) as shape_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
        ...
        expected = center_projection(
            ...
            reference_prev, reference_curr, 2)
        expected = shape_projection(
            expected[0], expected[1], reference_prev, reference_curr, 2)
        swapped = center_projection(
            residual_curr, residual_prev,
            reference_curr, reference_prev, 2)
        swapped = shape_projection(
            swapped[0], swapped[1], reference_curr, reference_prev, 2)
        self.assertTrue(torch.equal(expected[0][:, :2], residual_prev[:, :2]))
        self.assertTrue(torch.equal(expected[1][:, :2], residual_curr[:, :2]))
        self.assertTrue(torch.allclose(
    ...
    def test_body_frame_center_tangent_is_rotation_equivariant(self):
        projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_body_frame_center_tangent_residual)
        reference_prev = torch.tensor([[[
            0.40, 0.45, 0.20, 0.10, 0.15]]], device=self.device)
        reference_curr = torch.tensor([[[
            0.55, 0.50, 0.22, 0.11, 0.18]]], device=self.device)
        target_prev = torch.tensor([[[
            0.43, 0.46, 0.21, 0.10, 0.16]]], device=self.device)
        target_curr = torch.tensor([[[
            0.59, 0.53, 0.23, 0.12, 0.20]]], device=self.device)

        def residual(target, reference):
            return (torch.logit(target.clamp(1e-6, 1 - 1e-6))
                    - torch.logit(reference.clamp(1e-6, 1 - 1e-6)))

        projected = projection(
            residual(target_prev, reference_prev),
            residual(target_curr, reference_curr),
            reference_prev, reference_curr, 0)

        turn = 0.20
        theta = torch.tensor(turn * torch.pi, device=self.device)
        cos_theta, sin_theta = torch.cos(theta), torch.sin(theta)

        def rotate_boxes(boxes):
            centered = boxes[..., :2] - 0.5
            rotated_center = torch.stack((
                cos_theta * centered[..., 0]
                - sin_theta * centered[..., 1],
                sin_theta * centered[..., 0]
                + cos_theta * centered[..., 1]), dim=-1) + 0.5
            rotated_angle = torch.remainder(boxes[..., 4:] + turn, 1.0)
            return torch.cat((
                rotated_center, boxes[..., 2:4], rotated_angle), dim=-1)

        rotated_reference_prev = rotate_boxes(reference_prev)
        rotated_reference_curr = rotate_boxes(reference_curr)
        rotated_target_prev = rotate_boxes(target_prev)
        rotated_target_curr = rotate_boxes(target_curr)
        rotated_projected = projection(
            residual(rotated_target_prev, rotated_reference_prev),
            residual(rotated_target_curr, rotated_reference_curr),
            rotated_reference_prev, rotated_reference_curr, 0)

        def decode_center(projected_residual, reference):
            return (projected_residual[..., :2]
                    + torch.logit(reference[..., :2])).sigmoid()

        decoded_prev = decode_center(projected[0], reference_prev)
        decoded_curr = decode_center(projected[1], reference_curr)
        rotated_decoded_prev = decode_center(
            rotated_projected[0], rotated_reference_prev)
        rotated_decoded_curr = decode_center(
            rotated_projected[1], rotated_reference_curr)
        expected_prev = rotate_boxes(torch.cat((
            decoded_prev, reference_prev[..., 2:]), dim=-1))[..., :2]
        expected_curr = rotate_boxes(torch.cat((
            decoded_curr, reference_curr[..., 2:]), dim=-1))[..., :2]
        self.assertTrue(torch.allclose(
            rotated_decoded_prev, expected_prev, atol=2e-5, rtol=1e-5))
        self.assertTrue(torch.allclose(
            rotated_decoded_curr, expected_curr, atol=2e-5, rtol=1e-5))

    def test_se2_product_tangent_is_terminal_parameter_free_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        shape_projection = projected._pair_transport_shape_tangent_residual
        center_projection = projected._pair_transport_se2_center_tangent_residual
        call_order = []

        def record_shape(*args, **kwargs):
            call_order.append('shape')
            return shape_projection(*args, **kwargs)
        ...
        with mock.patch.object(
                projected,
                '_pair_transport_shape_tangent_residual',
                side_effect=record_shape) as shape_mock, mock.patch.object(
                    projected,
                    '_pair_transport_se2_center_tangent_residual',
                    side_effect=record_center) as center_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
                1, decoder=projected,
                reg_branches_prev=reg_prev, reg_branches_curr=reg_curr)
        self.assertEqual(shape_mock.call_count, 1)
        self.assertEqual(center_mock.call_count, 1)
        self.assertEqual(call_order, ['shape', 'center'])
        for reference in refs_prev + refs_curr:
            self.assertTrue(torch.isfinite(reference).all())

        torch.manual_seed(80405)
        reference_prev = (
        ...
        residual_curr = (
            torch.randn(2, 7, 5, device=self.device) * 0.2).requires_grad_()
        num_dn = 2
        shaped = shape_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, num_dn)
        expected = center_projection(
            shaped[0], shaped[1], reference_prev, reference_curr, num_dn)
        swapped_shaped = shape_projection(
            residual_curr, residual_prev,
            reference_curr, reference_prev, num_dn)
        swapped = center_projection(
            swapped_shaped[0], swapped_shaped[1],
            reference_curr, reference_prev, num_dn)
        self.assertTrue(torch.equal(
    ...
    def test_se2_center_tangent_reduces_to_body_frame_at_zero_angle(self):
        body_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_body_frame_center_tangent_residual)
        se2_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_se2_center_tangent_residual)
        torch.manual_seed(80505)
        reference_prev = (
            torch.rand(2, 6, 5, device=self.device) * 0.6 + 0.2)
        reference_curr = (
            torch.rand(2, 6, 5, device=self.device) * 0.6 + 0.2)
        shared_angle = (
            torch.rand(2, 6, 1, device=self.device) * 0.6 + 0.2)
        reference_prev[..., 4:] = shared_angle
        reference_curr[..., 4:] = shared_angle
        residual_prev = torch.randn(2, 6, 5, device=self.device) * 0.2
        residual_curr = torch.randn(2, 6, 5, device=self.device) * 0.2
        residual_prev[..., 4:] = 0.0
        residual_curr[..., 4:] = 0.0
        ...
        se2 = se2_projection(
            ...
            reference_prev, reference_curr, 1)
        self.assertTrue(torch.allclose(
            body[0], se2[0], atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            body[1], se2[1], atol=2e-6, rtol=1e-5))

    def test_se2_center_tangent_is_rotation_equivariant(self):
        projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_se2_center_tangent_residual)
        reference_prev = torch.tensor([[[
        ...
        reference_curr = torch.tensor([[[
            0.55, 0.50, 0.22, 0.11, 0.18]]], device=self.device)
        target_prev = torch.tensor([[[
            0.43, 0.46, 0.21, 0.10, 0.22]]], device=self.device)
        target_curr = torch.tensor([[[
            0.59, 0.53, 0.23, 0.12, 0.12]]], device=self.device)
    ...
    def test_frenet_product_tangent_is_terminal_parameter_free_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = (
            projected._pair_transport_frenet_center_tangent_residual)
        shape_projection = projected._pair_transport_shape_tangent_residual
        call_order = []
        ...
        with mock.patch.object(
                projected,
                '_pair_transport_frenet_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected,
                    '_pair_transport_shape_tangent_residual',
                    side_effect=record_shape) as shape_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
                1, decoder=projected,
                reg_branches_prev=reg_prev, reg_branches_curr=reg_curr)
        self.assertEqual(center_mock.call_count, 1)
        self.assertEqual(shape_mock.call_count, 1)
        self.assertEqual(call_order, ['center', 'shape'])
        for reference in refs_prev + refs_curr:
            self.assertTrue(torch.isfinite(reference).all())

        torch.manual_seed(80406)
        reference_prev = (
            torch.rand(2, 7, 5, device=self.device) * 0.6 + 0.2)
        reference_curr = (
            torch.rand(2, 7, 5, device=self.device) * 0.6 + 0.2)
        ...
        centered = center_projection(
            ...
            reference_prev, reference_curr, num_dn)
        expected = shape_projection(
            centered[0], centered[1], reference_prev, reference_curr, num_dn)
    ...
    def test_frenet_center_tangent_reduces_to_body_frame_at_zero_turn(self):
        body_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_body_frame_center_tangent_residual)
        frenet_projection = (
        ...
        body = body_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        frenet = frenet_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        self.assertTrue(torch.allclose(
            body[0], frenet[0], atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            body[1], frenet[1], atol=2e-6, rtol=1e-5))

    def test_shared_metric_product_tangent_is_terminal_parameter_free_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = (
            projected._pair_transport_shared_metric_center_tangent_residual)
        shape_projection = projected._pair_transport_shape_tangent_residual
        call_order = []
        ...
        with mock.patch.object(
                projected,
                '_pair_transport_shared_metric_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected,
                    '_pair_transport_shape_tangent_residual',
                    side_effect=record_shape) as shape_mock:
    ...
    def test_shared_metric_center_tangent_reduces_at_equal_size_and_is_active(
            self):
        product_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_center_tangent_residual)
        shared_metric_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_shared_metric_center_tangent_residual)
        torch.manual_seed(804081)
        reference_prev = (
            torch.rand(2, 6, 5, device=self.device) * 0.6 + 0.2)
        reference_curr = (
            torch.rand(2, 6, 5, device=self.device) * 0.6 + 0.2)
        shared_size = torch.rand(2, 6, 2, device=self.device) * 0.5 + 0.2
        reference_prev[..., 2:4] = shared_size
        reference_curr[..., 2:4] = shared_size
        residual_prev = torch.randn(2, 6, 5, device=self.device) * 0.2
        residual_curr = torch.randn(2, 6, 5, device=self.device) * 0.2
        product = product_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        shared_metric = shared_metric_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        self.assertTrue(torch.allclose(
            product[0], shared_metric[0], atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            product[1], shared_metric[1], atol=2e-6, rtol=1e-5))

        unequal_curr = reference_curr.clone()
        unequal_curr[..., 2:4] = (shared_size * 0.55).clamp_min(1e-3)
        product_unequal = product_projection(
            residual_prev, residual_curr,
            reference_prev, unequal_curr, 1)
        shared_metric_unequal = shared_metric_projection(
            residual_prev, residual_curr,
            reference_prev, unequal_curr, 1)
    ...
    def test_householder_product_tangent_is_terminal_norm_preserving_and_swap_equivariant(
        ...
        center_transport = (
            projected._pair_householder_center_tangent_residual)
        shape_transport = projected._pair_householder_shape_tangent_residual
        call_order = []

        def record_center(*args, **kwargs):
            call_order.append('center')
            return center_transport(*args, **kwargs)

        def record_shape(*args, **kwargs):
            call_order.append('shape')
            return shape_transport(*args, **kwargs)

        with mock.patch.object(
                projected,
                '_pair_householder_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected,
                    '_pair_householder_shape_tangent_residual',
                    side_effect=record_shape) as shape_mock:
            _, refs_prev, refs_curr, _, _ = self._forward(
                1, decoder=projected,
                reg_branches_prev=reg_prev, reg_branches_curr=reg_curr)
        ...
        residual_curr = (
            torch.randn(2, 7, 5, device=self.device) * 0.2).requires_grad_()
        num_dn = 2
        centered = center_transport(
            residual_prev, residual_curr,
            reference_prev, reference_curr, num_dn)
        expected = shape_transport(
            centered[0], centered[1], reference_prev, reference_curr, num_dn)
        swapped_centered = center_transport(
            residual_curr, residual_prev,
            reference_curr, reference_prev, num_dn)
        swapped = shape_transport(
            swapped_centered[0], swapped_centered[1],
            reference_curr, reference_prev, num_dn)
        self.assertTrue(torch.equal(
            expected[0][:, :num_dn], residual_prev[:, :num_dn]))
        ...
        detail = torch.randn(
            3, 5, 3, device=self.device, requires_grad=True)
        transport = torch.randn(3, 5, 3, device=self.device)
        transported = projected._householder_transport_detail(
            detail, transport)
        self.assertTrue(torch.allclose(
            transported.norm(dim=-1), detail.norm(dim=-1),
            atol=2e-6, rtol=1e-5))
        cosine = torch.nn.functional.cosine_similarity(
            transported, transport, dim=-1).abs()
        self.assertTrue(torch.allclose(
            cosine, torch.ones_like(cosine), atol=2e-5, rtol=1e-5))
        self.assertTrue(torch.allclose(
            projected._householder_transport_detail(-detail, -transport),
    ...
    def test_spherical_midpoint_center_log_shape_is_terminal_norm_preserving_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_transport = (
            projected._pair_spherical_midpoint_center_tangent_residual)
        size_consensus = projected._pair_shared_log_size_residual
        angle_consensus = projected._pair_shared_periodic_angle_residual
        call_order = []
        ...
        with mock.patch.object(
                projected,
                '_pair_spherical_midpoint_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected, '_pair_shared_log_size_residual',
                    side_effect=record_size) as size_mock, mock.patch.object(
                        projected, '_pair_shared_periodic_angle_residual',
        ...
        detail = torch.tensor(
            [[[1.0, 0.0], [0.0, 2.0]]], device=self.device,
            requires_grad=True)
        transport = torch.tensor(
            [[[0.0, 1.0], [3.0, 0.0]]], device=self.device)
        transported = projected._spherical_midpoint_transport_detail(
            detail, transport)
        root_half = 2.0**-0.5
        expected_transport = torch.tensor(
            [[[root_half, root_half], [2.0 * root_half,
                                       2.0 * root_half]]],
            device=self.device)
        self.assertTrue(torch.allclose(
            transported, expected_transport, atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            transported.norm(dim=-1), detail.norm(dim=-1),
            atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            projected._spherical_midpoint_transport_detail(
                -detail, -transport),
            -transported, atol=2e-6, rtol=1e-5))
        transported.sum().backward()
        self.assertTrue(torch.isfinite(detail.grad).all())
    ...
    def test_hemisphere_fold_center_log_shape_is_terminal_minimal_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_transport = (
            projected._pair_hemisphere_fold_center_tangent_residual)
        size_consensus = projected._pair_shared_log_size_residual
        angle_consensus = projected._pair_shared_periodic_angle_residual
        call_order = []
        ...
        with mock.patch.object(
                projected,
                '_pair_hemisphere_fold_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected, '_pair_shared_log_size_residual',
                    side_effect=record_size) as size_mock, mock.patch.object(
                        projected, '_pair_shared_periodic_angle_residual',
        ...
        detail = torch.tensor(
            [[[-2.0, 3.0], [2.0, 3.0]]], device=self.device,
            requires_grad=True)
        transport = torch.tensor(
            [[[1.0, 0.0], [1.0, 0.0]]], device=self.device)
        transported = projected._hemisphere_fold_transport_detail(
            detail, transport)
        expected_transport = torch.tensor(
            [[[2.0, 3.0], [2.0, 3.0]]], device=self.device)
        self.assertTrue(torch.allclose(
        ...
        self.assertTrue(torch.all(
            (transported * transport).sum(dim=-1) >= 0))
        self.assertTrue(torch.allclose(
            projected._hemisphere_fold_transport_detail(-detail, -transport),
            -transported, atol=2e-6, rtol=1e-5))
        transported.sum().backward()
        self.assertTrue(torch.isfinite(detail.grad).all())
    ...
    def test_axis_frenet_product_tangent_is_terminal_parameter_free_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = (
            projected._pair_transport_axis_frenet_center_tangent_residual)
        shape_projection = projected._pair_transport_shape_tangent_residual
        call_order = []
        ...
        with mock.patch.object(
                projected,
                '_pair_transport_axis_frenet_center_tangent_residual',
                side_effect=record_center) as center_mock, mock.patch.object(
                    projected,
                    '_pair_transport_shape_tangent_residual',
                    side_effect=record_shape) as shape_mock:
    ...
    def test_axis_frenet_center_tangent_reduces_to_product_at_zero_turn(self):
        product_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_center_tangent_residual)
        axis_frenet_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_axis_frenet_center_tangent_residual)
        ...
        product = product_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        axis_frenet = axis_frenet_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        self.assertTrue(torch.allclose(
            product[0], axis_frenet[0], atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            product[1], axis_frenet[1], atol=2e-6, rtol=1e-5))

    def test_covariant_frenet_product_tangent_is_terminal_parameter_free_and_swap_equivariant(
            self):
        parent, _, _ = _build_decoder(num_layers=4, device=self.device)
        projected, reg_prev, reg_curr = _build_decoder(
            num_layers=4,
        ...
        center_projection = (
            projected.
            _pair_transport_covariant_frenet_center_tangent_residual)
        shape_projection = projected._pair_transport_shape_tangent_residual
        call_order = []
    ...
    def test_covariant_frenet_center_tangent_reduces_to_product_at_zero_turn(
            self):
        product_projection = (
            PairRotatedRTDETRTransformerDecoder.
            _pair_transport_center_tangent_residual)
        ...
        product = product_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        covariant = covariant_projection(
            residual_prev, residual_curr,
            reference_prev, reference_curr, 1)
        self.assertTrue(torch.allclose(
            product[0], covariant[0], atol=2e-6, rtol=1e-5))
        self.assertTrue(torch.allclose(
            product[1], covariant[1], atol=2e-6, rtol=1e-5))
